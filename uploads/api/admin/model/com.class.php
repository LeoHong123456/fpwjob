...
class com_controller extends appadmin
{
	...
	function oncedel_action()
	{
		...
		$this->write_appadmin_log("删除店铺招聘(ID:".$_POST['ids'].")");
		$data['error']=1;
		echo json_encode($data);die;
	}
	...
	function oncestatus_action()
	{
		...
		$this->write_appadmin_log("审核店铺招聘(ID:".$_POST['ids'].")");
		$data['error']=1;
		echo json_encode($data);die;
	}
		...
		if(!empty($rows))
		{
			...
			foreach($rows as $k=>$v)
			{
				foreach($statis as $val)
				{
					if($v['uid']==$val['uid'])
					{
						$list[$k]['rating_name']=$v['rating_name'];
					}
				}
				$list[$k]['uid']=$v['uid'];
	...
	function companyshow_action()
	{
		...
		if(!empty($row))
		{
			...
			$list['address']	=$row['address'];
			$list['linkman']	=$row['linkman'];
			$list['linkjob']	=$row['linkjob'];
			$list['linkmail']	=$row['linkmail'];
			$list['hy']			=$row['hy'];
			$list['pr']			=$row['pr'];
			$list['provinceid']=$row['provinceid'];
			$list['cityid']	=$row['cityid'];
			...
			$list['zip']		=$row['zip'];
			$list['linkqq']	=$row['linkqq'];
			$list['linkphone']=$row['linkphone'];
	...
	function companystatus_action()
	{
		...
		if(!empty($member))
		{
			...
			foreach($member as $v)
			{
				$notice->sendEmailType(array("uid"=>$v['uid'],"name"=>$v['name'],"email"=>$v['email'],"status_info"=>$_POST['statusbody'],"date"=>date("Y-m-d H:i:s"),"type"=>"userstatus"));
			}
	...
	function lock_action()
	{
		...
		$notice = $this->MODEL('notice');
    $notice->sendEmailType(array("email"=>$email[0]['email'],"uid"=>$_POST['uid'],"name"=>$email[0]['name'],"lock_info"=>$_POST['statusbody'],"type"=>"lock"));
		if($id)
		{
