...
class order_controller extends appadmin
{
	function list_action()
	{
		...
		if(!empty($rows))
		{
			...
			foreach($rows as $k=>$v)
			{
				...
				$lt=$this->obj->DB_select_all("lt_info","`uid` in (".pylode(",",$uid).")","`uid`,`realname`");
	...
	function show_action()
	{
		...
		if(!empty($row))
		{
			...
			if($row['is_invoice']=="1")
			{
				...
				$list['link_man']=$invoice['link_man'];
				$list['link_moblie']	=$invoice['link_moblie'];
				$list['address']=$invoice['address'];
	...
	function save_action()
	{
		...
		if($_POST['is_invoice']=='0')
		{
			...
			$value="`invoice_id`='".$this->stringfilter($_POST['invoice_id'])."',";
	...
	function del_action()
	{
		...
		if($company_order&&is_array($company_order))
		{
			...
			$this->obj->DB_delete_all("invoice_record","`order_id` in(".pylode(',',$order_ids).")","");
