...
class user extends common{
	...
	function __construct($tpl,$db,$def="",$model="index",$m="") {
		...
		if($this->config['user_resume_status']=='1' && $_GET['c']!='expect' && $_GET['c']!='passwd' && $_GET['act']!='logout' && $_GET['c']!='binding' && $_GET['c']!='log'){
	...
	function public_action(){
		...
		$user=$this->obj->DB_select_once("resume","`uid`='".$this->uid."'","`name`,`sex`,`photo`,`resume_photo`,`photo_status`,`idcard_pic`,`idcard_status`");
		...
		if($_GET['c']==''){
			...
			$this->yunset("left",2);
		}elseif($_GET['c']=='commsg'||$_GET['c']=='subscribe'||$_GET['c']=='finder'||$_GET['c']=='favorite'||$_GET['c']=='look_job'||$_GET['c']=='job'||$_GET['c']=='msg'||$_GET['c']=='atn'||$_GET['c']=='partapply'||$_GET['c']=='partcollect'||$_GET['c']=='rebates'||$_GET['c']=='atnlt'||$_GET['c']=='xjh'||$_GET['c']=='academy'||$_GET['c']=='comment' ||$_GET['c']=='atn_teacher'||$_GET['c']=='fav_subject'||$_GET['c']=='fav_agency'||$_GET['c']=='subject_zixun'||$_GET['c']=='baoming_subject'|| ($_GET['c']=='jobpack' && !$_GET['act'])){
			
			$this->yunset("left",3);
		}elseif($_GET['c']=='paylist'||$_GET['c']=='paylog'|| $_GET['c']=='integral' || $_GET['c']=='integral_reduce' || $_GET['c']=='reward_list' || $_GET['c']=='pay' || $_GET['c']=='payment'||$_GET['m']=='invitereg' || $_GET['act']=='loglist' || $_GET['act']=='withdrawlist' || $_GET['act']=='withdraw'|| $_GET['act']=='change'|| $_GET['act']=='changelist' ){
			
			$this->yunset("left",5);
		}elseif($_GET['c']=='setname'||$_GET['c']=='passwd'||$_GET['c']=='binding'||$_GET['c']=='sysnews'){ 
			$this->yunset("left",6);
		}
	}
	function member_satic(){
		...
		if($statis['integral']==''){$statis['integral']=0;}
