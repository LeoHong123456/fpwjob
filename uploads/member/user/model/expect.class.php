...
class expect_controller extends user{
	function index_action(){
		...
		if($_GET['e']){
			...
			$show = $this->obj->DB_select_all("resume_show","`uid`='".$this->uid."' and `eid` ='".$eid."'");
				...
				if($nid){
					...
					if ($brforeresume){
					    ...
					    if($citynochange==1){
					        ...
					        $return=$resumeM->addCityclass($eid,$this->uid,$_POST['city_classid']);
					    }
	...
	function add_action(){
		if($_POST['submit']){
			...
			if($_POST['birthday']==""){$this->ACT_layer_msg("请选择出生年月！",8);}
			...
			if ($nid){
				...
				$resumeM->addCityclass($nid,$this->uid,$_POST['city_class']);
				$resumeM->addJobclass($nid,$this->uid,$_POST['job_class']);
	...
	function success_action(){
		if($_GET['e']){
			...
			$resume=$this->obj->DB_select_once("user_resume","`eid`='".(int)$_GET['e']."'");
	...
	function showresume_action(){
	    ...
	    if($_POST['eid']){
	        ...
	        if($table=='resume_other'){
	            ...
	            if(is_array($info)){
	                foreach ($info as $v){
	                    $html.="<li class='yun_resume_exp_list' id='other".$v['id']."'><i class='resume_skill_icon'></i>标题：<span class='resume_table_blod'>".$v['name']."</span>";
						if($v['content']){
							$html.="<div class='yun_resume_exp_p'>内容：".$v['content']."</div>";
						}
						$html.="</li>";
	                }
	                echo $html;die;
