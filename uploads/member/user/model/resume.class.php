...
class resume_controller extends user{
	function index_action(){
		...
		$rows=$this->obj->DB_select_all("resume_expect","`uid`='".$this->uid."' order by defaults desc,lastupdate desc","id,name,lastupdate,doc,tmpid,integrity,hits,height_status,statusbody,`topdate`,`is_entrust`,`top`,`r_status`,`status`");
    ...
    function defaultresume_action(){
		...
		if($row['id']){
			...
			$this->obj->member_log("设置默认简历",2,2);
		}
		$this->layer_msg('操作成功！',9,0,"index.php?c=resume");
	}
}
