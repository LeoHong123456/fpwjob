	...
	function index_action(){
		...
		if(is_array($resume_expect)&& !empty($resume_expect)){
			...
			foreach($resume_expect as $key=>$val){
				...
				$html.="<tr class=\"result_class\"><td>".mb_substr($val['name'],0,8,"utf-8")."</td><td>".$telphone['telphone']."</td><td><span>".$doc_type."</span></td><td>".date('Y-m-d',$val['lastupdate'])."</td><td>".$status."</td><td>".$entrust."</td></tr>";
			}
			echo $html;die;
		}else{
			echo 1;die;
			...
			if($resume_expect['is_entrust']=='0'){
				...
				if($entrust_num<(int)$this->config['user_trust_number']){
					...
					if($member_statis['pay'] < $this->config['pay_trust_resume'] && $this->config['pay_trust_resume']){
						$data['type']='8';
						$data['msg']='余额不足，无法委托！';
						$data['url']='index.php?c=pay&act=money';
					}else{						
						$res=$IntegralM->company_invtal($this->uid,$this->config['pay_trust_resume'],false,"委托简历",true,2,'pay'); 						
						if($res){
							$idata['uid']      = $this->uid;
				...
				if($user_entrust['id']){
					...
					if($res){
						if($user_entrust['status']=='0'){
							$IntegralM->company_invtal($this->uid,$user_entrust['price'],true,"退还委托简历费用",true,2,'pay');   
						}
