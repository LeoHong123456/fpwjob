...
class tongji_controller extends company{
	function index_action(){
		...
		$useridjob=$this->obj->DB_select_all("userid_job",$where,"job_id,eid");
		...
		if($_GET['type']=="1"){
			...
			$list = $this->obj->DB_select_all("company_job","`id` in (".@implode(",",$job_id).") group by provinceid","count(*) as num,provinceid");
			if(is_array($list)){
