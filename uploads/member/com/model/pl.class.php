...
class pl_controller extends company{
	function index_action(){
		...
		if(is_array($rows)){
			...
			$uid=pylode(",",$uid);
			$user=$this->obj->DB_select_all("resume","`uid` in ($uid)","`uid`,`name`,`def_job`");
			foreach($rows as $k=>$v){
				foreach($user as $val){
					if($v['uid']==$val['uid']){
	...
	function save_action(){
		if($_POST['submit']){
			...
			$nid=$this->obj->update_once("company_msg",$data,$where);
 			if($nid){
 				$this->obj->member_log("回复企业评论",18,2);
	...
	function plset_action(){
		if($_POST['ajax']=="1" && $_POST['id']){
			...
			if($id){
				$this->obj->member_log("设置企业评论审核状态",18,2);
				echo 1;die;
			}else{
				echo 2;die;
			}
		...
		if($_POST['status'] && $_POST['id']){
			...
			if($nid){
				$this->obj->member_log("审核企业评论",18,2);
				echo 1;die;
			}else{
				echo 2;die;
			}
	...
	function del_action(){
		if($_GET['id']){
			...
			$nid=$this->obj->DB_delete_all("company_msg","`id`='".$del."' and `cuid`='".$this->uid."'");
 			if($nid){
 				$this->obj->member_log("删除企业评论",18,3);
