...
class hr_controller extends company{
	function index_action(){
		...
		if(trim($_GET['keyword'])){
			$resume=$this->obj->DB_select_all("resume","`r_status`<>'2' and `name` like '%".trim($_GET['keyword'])."%'","`name`,`edu`,`uid`,`exp`,`sex`,`nametype`");
		...
		if(is_array($rows) && !empty($rows)){
			...
			if(empty($resume)){
				$resume=$this->obj->DB_select_all("resume","`r_status`<>'2'  and `uid` in (".pylode(",",$uid).")","`def_job` as `id`,`name`,`edu`,`uid`,`exp`,`sex`,`nametype`");
			}
	...
	function hrset_action(){
		if($_POST['ajax']==1 && $_POST['ids']){
			...
			$this->obj->member_log("批量阅读申请职位的人才",6,2);
