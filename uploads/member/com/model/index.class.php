...
class index_controller extends company{
	function index_action(){
		...
		$member=$this->obj->DB_select_once("member","`uid`='".$this->uid."'","`login_date`,`status`,`wxid`,`wxopenid`");
		...
		$this->cookie->SetCookie("comwxbd",'1',($time + 86400));
	...
	function resumeajax_action(){
	    ...
	    $joblist=$this->obj->DB_select_all("company_job",$jobwhere,"`job1_son`,`job_post`,`provinceid`,`cityid`,`three_cityid`");
      
	    $blacklist=$this->obj->DB_select_all("blacklist","`p_uid`='".$this->uid."'","`c_uid`");
		if(is_array($joblist) && !empty($joblist)){
	        foreach($joblist as $v){
             $where[]="(FIND_IN_SET('".$v['provinceid']."',city_classid) or FIND_IN_SET('".$v['cityid']."',city_classid) or FIND_IN_SET('".$v['three_cityid']."',city_classid)) AND (FIND_IN_SET('".$v['job1_son']."',job_classid) or FIND_IN_SET('".$v['job_post']."',job_classid))";
	        }
	    ...
	    $resume = $this->obj->DB_select_all('resume_expect',"`uname`<>'' and `lastupdate`>'".strtotime("-180 day")."' and status<>'2' and `r_status`='1' and `job_classid`<>'' and `open`='1' and `defaults`=1".$whereSql.$blistSql." ORDER BY `lastupdate` DESC"); 
      $list=array();
	    if ($resume){
