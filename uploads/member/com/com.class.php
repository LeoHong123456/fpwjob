...
class company extends common{
	...
	function public_action(){
		...
		if(is_array($rating_1)){
			foreach($rating_1 as $v){
				$couponid[]=$v['coupon'];
			}
			if(empty($coupon)){
			...
			if(is_array($coupon)){
				foreach($rating_1 as $k=>$v){
					foreach($coupon as $val){
						if($v['coupon']==$val['id']){
							$rating_1[$k]['couponname']=$val['name'];
						}
					}
				}
			}
		...
		if(is_array($rating_2)){
			foreach($rating_2 as $v){
				$couponid[]=$v['coupon'];
			}
			if(empty($coupon)){
				$coupon=$this->obj->DB_select_all("coupon","`id` in (".@implode(",",$couponid).")","`id`,`name`");
			...
			if(is_array($coupon)){
				foreach($rating_2 as $k=>$v){
					foreach($coupon as $val){
						if($v['coupon']==$val['id']){
							$rating_2[$k]['couponname']=$val['name'];
						}
					}
				}
			}
			...
			foreach($added as $key=>$val){
				foreach($service_detail as $v){
					if($val['id'] == $v['type']){
						...
						$added[$key]['lt_job_num'][]= $v['lt_job_num'];
						$added[$key]['lt_breakjob_num'][]= $v['lt_breakjob_num'];
						$added[$key]['lt_resume'][]= $v['lt_resume'];
 					} 
				}
			}
 		}
	...
	function company_satic(){
		...
		if($statis['vip_etime']>time() || $statis['vip_etime']==0){
			if($statis['rating_type']=="2"){
				...
				$addltjobnum=$addjobnum=$addpartjobnum='0';
			}
		}else {
	...
	function job(){
		if($_GET['r_uid']){
			...
			if(is_array($haves)){
				...
				if($nid){
					$this->obj->member_log("举报人才《".$_GET['r_name']."》",23,1);
		...
		if($_POST['status'] && ($_POST['id']|| is_array($_POST['id']))){
			...
			if($nid){
				...
				$this->obj->member_log("修改职位发布状态",1,2);
				echo 1;die;
			}else{
				echo 2;die;
			}
	...
	function part(){
		if($_POST['gotimeid']){
			...
			if($_POST['day']<1){
				...
				if($id)
				{
					$this->obj->member_log("兼职职位延期",9,2);
		...
		if($_POST['recid']){
			...
			$this->obj->member_log("发布推荐兼职职位《".$part['name']."》",9,1);
