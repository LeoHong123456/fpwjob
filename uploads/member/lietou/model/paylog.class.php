...
class paylog_controller extends lietou{
	function index_action(){
		...
		$this->public_action();
		$where="`uid`='".$this->uid."' and `order_price`>0  order by `id` desc";
		...
		if($rows&&is_array($rows)){
			foreach($rows as $key=>$val){
				...
				$rows[$key]['zf_type']=$arr_data['pay'][$val['order_type']];
			}
		}
	...
	function del_action(){
		if($this->usertype!='3' || $this->uid==''){
			...
			$this->obj->DB_delete_all("invoice_record","`oid`='".(int)$_GET['id']."'  and `uid`='".$this->uid."'");
			if($oid){
