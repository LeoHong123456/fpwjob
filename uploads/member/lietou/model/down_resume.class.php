...
class down_resume_controller extends lietou{
 	function index_action(){
 	    ...
 	    if(is_array($list)&&$list){
            ...
            $resume=$this->obj->DB_select_alls("resume","resume_expect","a.uid=b.uid and a.`r_status`<>'2' and a.uid in (".@implode(",",$uid).")","a.`name`,a.`uid`,b.`hy`,b.`cityid`,b.`job_classid`,b.`city_classid`,b.`lastupdate`,b.`height_status`");
 	        if(is_array($resume)){
 	            ...
 	            foreach($list as $key=>$val){
 	                foreach($resume as $va){
 	                    if($val['uid']==$va['uid']){
 	                        ...
 	                        if($va['city_classid']!=""){
 	                            $city_classid=@explode(",",$va['city_classid']);
 	                            $list[$key]['citylist']=$city_name[$city_classid[0]];
 	                        }
 	                    }
 	                }
 	            }
