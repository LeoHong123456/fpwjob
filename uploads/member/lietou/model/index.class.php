				...
				foreach ($resumeexp as $v){
					if($val['eid']=$v['id']){
						...
						if($v['city_classid']!=""){
						    ...
						    $yqresume[$k]['cityclassname']=$citylist['0'];
						}
					}
				}
			}
				...
				foreach($recresume as $k=>$val){
					...
					foreach($temporary as $v){
						if($v['rid']==$val['id']){
							...
							if($city_name[$v['provinceid']]){
								$recresume[$k]['cityname'].=$city_name[$v['provinceid']];
							}
							if($city_name[$v['cityid']]){
								$recresume[$k]['cityname'].='-'.$city_name[$v['cityid']];
							}
							if($city_name[$v['three_cityid']]){
								$recresume[$k]['cityname'].='-'.$city_name[$v['three_cityid']];
							}
