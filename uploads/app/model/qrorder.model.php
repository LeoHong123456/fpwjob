 		...
 		if($order['order_state']!='2'){
			...
			if($order['type']=='1' && $order['rating'] && $usertype['usertype']!='1'){
				...
				if($usertype['usertype']=='2'){
					...
					$this->DB_update_all("user_log","`status`=1","`orderid`='".$order['order_id']."'");
				}else if($usertype['usertype']=='3'){
				...
				$ratinginfo = $this->DB_select_once("company_rating","`id`='".$order['rating']."'");
				$this->DB_update_all("hotjob","`rating_id`='".$order['rating']."',`rating`='".$ratinginfo['name']."'","`uid`='".$order["uid"]."'");
				...
				$px = $this->DB_select_once("px_baoming","`id`='".$order['sid']."'","`s_uid`");
				
				if($px){
					
					$subject = $this->obj->DB_select_once("px_subject","`id`='".$px['sid']."'","`name`");

					$nid = $this->DB_update_all("px_train_statis","`packpay`=`packpay`+'".$order['order_price']."'","`uid`='".$px['s_uid']."'");					
					$nid = $this->DB_insert_once("company_pay","`order_id`='".$order['order_id']."',`order_price`='".$order['order_price']."',`pay_time`='".time()."',`pay_state`='2',`com_id`='".$px["s_uid"]."',`pay_remark`='课程《'".$subject['name']."'》报名费',`type`='2',`did`='".$this->config['did']."'");
				...
				if($order_info['jobid']){
					...
					if(!empty($packjob)){
						...
						if($packjob['packnum']<1){
							...
							$status = $this->DB_update_all("company_job_share","`packnum`='".$order_info['packnum']."',`packprice`='".$order_info['packprice']."',`packmoney`='".$order_info['packmoney']."'","`id`='".$packjob['id']."'");
							$shareType = 1;
						...
						$job = $this->DB_select_once('company_job',"`id`='".$order_info['jobid']."'","`status`");
						if(!empty($job)){
				...
				if($order['order_id']){
					$ad = $this->DB_select_once("ad_order","`order_id`='".$order['order_id']."' ","`price`");
					if(!empty($ad)){
	 					$status=$this->update_once("ad_order",array('order_state'=>'2'),"`order_id`='".$order['order_id']."'");
					}
				}
 			}
			if($status){
				if($this->config['sy_msg_fkcg']=='1'||$this->config['sy_email_fkcg']=='1'){
					...
					}elseif($member['usertype']=='4'){
						$fdata=$this->DB_select_once("px_train","`uid`='".$member['uid']."'","`name`,`uid`");
					}
 					$data=array();
