 		...
 		if($order['order_state']=='1'){
			...
			if($order['type']=='1' && $order['rating'] && $usertype['usertype']!='1'){
				...
				if($px){
					...
					$nid = $this->DB_insert_once("company_pay","`order_id`='".$order['order_id']."',`order_price`='".$order['order_price']."',`pay_time`='".time()."',`pay_state`='2',`com_id`='".$px["s_uid"]."',`pay_remark`='课程报名费',`type`='2',`did`='".$this->config['did']."'");
