...
class comtc_model extends model{
	...
	function invite_resume($data){
		...
		if($data['uid']=='' || $data['usertype']==''){
			$return['status'] = 7;
		}else{
		    if($data['usertype']!='2'){
		        $typename=array('1'=>'个人账户','2'=>'企业账户','3'=>'猎头账户','4'=>'培训账户');
		        $return['typename'] = $typename[$data['usertype']];
		        $return['username'] = $data['username'];
		        $return['typeurl'] = Url('wap',array('c'=>'ajax','a'=>'notuserout'));
		        $return['status'] = 6;
		    }else{
		        $member = $this->DB_select_once("member","`uid`='".$data['uid']."'","`status`");
		        $company = $this->DB_select_once("company","`uid`='".$data['uid']."'","`linktel`,`linkphone`,`linkman`,`address`");
		        if($member['status'] != 1){
		            $return['status'] = 5;
		        }else if($show_job){
		            if($jobtype=='2'){
		                $company_job=$this->DB_select_all("lt_job"," `uid`='".$data['uid']."' and `status` = '1' and `zp_status` ='0'","`job_name` as `name`,`id`");
		            }else{
		                $company_job=$this->DB_select_all("company_job","`uid`='".$data['uid']."' and `state`='1' and `r_status` <> '2' and `status` <> '1'","`name`,`id`,`is_link`,`link_type`");
		            }
		            if($company_job && is_array($company_job)){
		                $joblink = $this->DB_select_once("company_job_link","`jobid`='".$jobid."' and `uid`='".$data['uid']."'","`link_man`,`link_moblie`");
		...
		if($return['status']==''){
			...
			$row = $this->DB_select_once("company_statis","`uid`='".$data['uid']."'","`rating`,`vip_etime`,`invite_resume`,`rating_type`");
			if($row['vip_etime'] > time() || $row['vip_etime'] == '0'){

				if($this->config['integral_interview']=='0' && $row['invite_resume']=='0'){
					$this->DB_update_all("company_statis","`invite_resume`='1'","`uid`='".$data['uid']."'");
					$row = $this->DB_select_once("company_statis","`uid`='".$data['uid']."'","`rating`,`vip_etime`,`invite_resume`,`rating_type`");
					...
					if($statis['rating_type']=='1'){
						if($statis['breakjob_num'] >= $jobnum){
							...
							if($nid){
								...
								if($jobnum == 1){
								    $this->member_log("刷新职位《".$jobs[0]['name']."》",1,4,$data['uid'],2);
								}else{
								    $this->member_log("批量刷新职位",1,4,$data['uid'],2);
								}
								$return['status']='1';
								$return['msg']='职位刷新成功';
							}else{
							...
							if($this->config['com_integral_online']=='4'){
								...
								$return['status']='2';
								$return['msg']='刷新套餐不足，是否继续刷新？<br>您还可以<a href="index.php?c=right&act=added" style="color:red;">购买增值包</a>！';
							}
						}
					...
					if($statis['rating_type']=='1'){
						if($statis['breakpart_num'] >= $partnum){
							...
							if($nid){
								...
								if($partnum == 1){
								    $this->member_log("刷新兼职《".$parts[0]['name']."》",9,4,$data['uid'],2);
								}else{
								    $this->member_log("批量刷新兼职",9,4,$data['uid'],2);
								}
								$return['status']='1';
								$return['msg']='兼职刷新成功';
							}else{
		...
		if($data['ltjobid']){
			...
			if(empty($ltjobs)){
				...
				if($statis['vip_etime'] > time() || $statis['vip_etime'] == '0'){
					...
					if($this->config['integral_lt_jobefresh']=='0' && $statis['lt_breakjob_num']=='0'){
					...
					if($statis['rating_type']=='1'){
						...
						if($this->config['integral_lt_jobefresh']=='0'){
							$nid = $this->DB_update_all("lt_job","`lastupdate`='".time()."'","`uid`='".$this->uid."' and `id`in (".$jobid.") ");
							if($nid){
								$this->DB_update_all("company","`jobtime`='".time()."'","`uid`='".$this->uid."'");
								if($jobnum == 1){
