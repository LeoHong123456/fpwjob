...
class lietou_model extends model{
	...
	function GetLtNum($Where=array(),$Options=array()){
        $WhereStr=$this->FormatWhere($Where);
        $FormatOptions=$this->FormatOptions($Options);
        return $this->DB_select_num('lt_info',$WhereStr,$FormatOptions['field']);
    }
	function GetLtStatisAll($Where=array(),$Options=array('field'=>null)){
		$WhereStr=$this->FormatWhere($Where);
		$FormatOptions=$this->FormatOptions($Options);
		return $this->DB_select_all('lt_statis',$WhereStr,$FormatOptions['field']);
	}
