...
class weixin_model extends model{
	...
	function myMsg($wxid='')
	{
		...
		{
			$Return['centerStr'] = "<Content><![CDATA[您最近没有新的消息！]]></Content>";
	...
	function Audition($wxid='')
	{
		...
		if($userBind['bindtype']=='1')
		{
			$num = $this->DB_select_num("userid_msg","`uid`='".$userBind['uid']."'");
			
			if($num>0)
			{
				$Msg['title'] = '近期有'.$num.'家企业向您发出面试邀请，点击查看';
	...
	function ApplyJob($wxid='')
	{
		...
		if($userBind['bindtype']=='1')
		{
			...
			$num = $this->DB_select_num("userid_job","`com_id`='".$userBind['uid']."' AND `is_browse`='1'");
			
			if($num>0)
			{
				$Msg['title'] = '近期有'.$num.'位人才向贵公司投递了简历，点击查看';
	...
	function PartApply($wxid='')
	{
		...
		if($userBind['bindtype']=='1')
		{
			...
			$num = $this->DB_select_num("part_apply","`comid`='".$userBind['uid']."' AND `status`='1'");
			
			if($num>0)
			{
				$Msg['title'] = '点击查看'.$num.'条 兼职报名信息';
	...
	function lookResume($wxid='')
	{
		...
		if($userBind['bindtype']=='1')
		{
			$num = $this->DB_select_num("look_resume","`uid`='".$userBind['uid']."'  AND `com_id`>0");
			if($num>0)
			{
				
					$Msg['title'] = $num.'家企业看过您的简历,点击查看';
	...
	function searchJob($keyword)
	{
		...
		if($keyword)
		{
			...
			if(is_array($keywords))
			{
				...
				$jobNum = $this->DB_select_num("company_job",$searchWhere."");
			}
		}	
		...
		if($jobNum>0)
		{
			...
			$Msg['title'] = '点击查看'.$jobNum.'条 与【'.$keyword. '】相关的职位';
	...
	function recJob()
	{
		...
		$jobNum = $this->DB_select_num("company_job","`sdate`<='".time()."' AND `status`<>1 AND `r_status`<>2 AND `rec_time`>'".time()."'");
		
		if($jobNum>0)
		{
			$Msg['title'] = '点击查看'.$jobNum.'条 推荐职位';
			
			$Msg['pic'] = $this->config['sy_weburl'].'/'.$this->config['sy_wx_logo'];
			
			
			$Msg['url'] = Url('wap',array('c'=>'job','rec'=>1));
	...
	function sendWxReward($data){
	   global $config;
		...
		if($data['wxid'])
		{
			...
			$TempDate['first']	= array('value'=>$First,'color'=>'');
			$TempDate['keyword1']	= array('value'=>$jobName,'color'=>'');
			$TempDate['keyword2']	= array('value'=>$rinfo,'color'=>'');
