...
class cache_model extends model{
        ...
        foreach($CacheTypeList as $v){
            switch($v){
                ...
                case 'hy':
                    ...
                    break;
                case 'school':
                    $ReturnCacheList=array_merge_recursive($ReturnCacheList,$this->school_cache($Options));
                    break;
                case 'lt':
    ...
    private function school_cache($Options=array('needreturn'=>false,'needassign'=>true)){
		include PLUS_PATH."/school.cache.php";
        if($Options['needreturn']==true){
            return array('schoolata'=>$schooldata,'schoolclass_name'=>$schoolclass_name);
        }
	}
