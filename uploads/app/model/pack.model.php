...
class pack_model extends model{
	...
	function getShareJob(){
		$where = "`sharepack`='1' AND `state`=1 and `r_status`<>2 and `status`<>1 ";
	...
	function delrewardJob($uid,$jobid){
		...
		if($reward['id']){
			...
			$rewardJobNum = $this->DB_select_num("company_job_rewardlist","`comid`='".(int)$uid."' AND `jobid`='".(int)$jobid."' AND status NOT IN ('0','8','18','19','20','21','23','26','27','28','29')");
			if($rewardJobNum>0){
				$data['error'] = '1';
	...
	function getShareJobOne($id){
		$where = "`id`='".(int)$id."' AND `sharepack`='1' AND `state`=1 and `r_status`<>2 and `status`<>1 ";
	...
	function redPackOrder($data){
		...
		if($data['jobid'] && $data['packmoney'] && $data['packnum']){
			...
			if($packmoney>0 && $packnum>0){
				...
				$job = $this->DB_select_once("company_job","`uid`='".$data['uid']."' AND `id`='".$data['jobid']."'","`id`,`uid`,`sharepack`");
				if(empty($job)){
	...
	function rewardPackOrder($data){
		...
		if($data['jobid'] && $data['rewardid']){
		    ...
		    $reward = $this->DB_select_once("company_job_rewardlist","`id`='".(int)$data['rewardid']."' AND `comid`='".$data['uid']."' AND `jobid`='".(int)$data['jobid']."'");
			
			if($reward['money']>0){
				
			    $job = $this->DB_select_once("company_job","`uid`='".$data['uid']."' AND `id`='".$data['jobid']."'","`id`,`uid`");
				if(empty($job)){
	...
	function rewardJob($data){
		...
		if($data['jobid']){
			if(($data['sqmoney']+$data['invitemoney']+$data['offermoney'])>1){
			    ...
			    $job = $this->DB_select_once("company_job","`uid`='".$data['uid']."' AND `id`='".$data['jobid']."'","`id`,`uid`,`rewardpack`");
				if(empty($job)){
					$return['error'] = '请选择正确的推广职位！';
				...
				if($usertype==1){
					...
					if(!$return['msg']){
						...
						if($this->config['sy_reward_sqnum']>0 && $sqNum>=$this->config['sy_reward_sqnum']){
							...
							$where = "`id`='".(int)$jobid."' AND `rewardpack`='1' AND `state`=1 and `r_status`=1 and `status`=0 ";
	...
	function logStatus($rewardid,$status,$uid,$utype,$data=array()){
		...
		if($rewardid && $status){
			...
			if(!empty($rewardInfo)){
				...
				if(in_array($rewardInfo['status'],array('18','19','20','21','23','27','28','29'))){
	...
	function withDraw($uid,$usertype,$money,$realname){
		...
		if(!$realname || !$uid || !$usertype || !$money){
			...
			$withNum = $this->DB_select_num("member_withdraw","`uid`='".$uid."' and `time`>='".strtotime(date('Ymd'))."'");
