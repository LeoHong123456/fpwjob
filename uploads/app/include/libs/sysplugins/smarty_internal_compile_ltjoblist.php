			...
			foreach('.$name.' as $k=>$v){
				...
				if($v[\'usertype\']==3){
					...
					'.$name.'[$k]["job_url"] = Url("lietou",array("c"=>"jobshow","id"=>$v[\'id\']));
					 '.$name.'[$k]["wap_lt_url"] = Url("wap",array("c"=>"ltjob","a"=>"hunter","uid"=>$v[uid]));
				}else{
				...
				if($v[\'minsalary\']>0&&$v[\'maxsalary\']>0){
                    ...
                    '.$name.'[$k]["salary_info"] = "￥".floatval($v[\'minsalary\'])."万以上";  
                }else{
                ...
                foreach($ltlist as $val){
					if($v[\'uid\']==$val[\'uid\']){
                        ...
                        if(!$val[\'photo_big\'] || !file_exists(str_replace($config[\'sy_weburl\'],APP_PATH,\'.\'.$val[\'photo_big\']))){
                            ...
                            '.$name.'[$k][\'logo_n\'] = str_replace("./",$config[\'sy_weburl\']."/",$val[\'photo_big\']);
                        } 
