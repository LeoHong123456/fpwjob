 ...
 */
 class cache{
	...
	public function lt_cache($dir){
    	...
    	public function school_cache($dir){
		$rows=$this->obj->DB_select_all("schoolclass","1 order by sort asc");
		if(is_array($rows)){
			foreach($rows as $v){
				if($v["keyid"]!=0){
					$school_index[$v["keyid"]][]=$v["id"];
				}
				$schoolname[$v['id']]=$v['name'];
			}
			foreach($rows as $v){
				if($v['keyid']==0){
					$data2[$v['variable']]=$school_index[$v['id']];
				}
			}
		}
        
		$data['schooldata']=ArrayToString($data2);
		$data['schoolclass_name']=ArrayToString($schoolname);
		return made_web_array($this->cachedir.$dir,$data);
	}
	...
	public function emailconfig_cache($dir){
		...
		$adminconfig=$this->obj->DB_select_num("admin_config","`name`='sy_email_set'");
		...
		$adminconfig=$this->obj->DB_select_all('admin_config');
