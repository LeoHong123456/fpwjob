	...
	function index_action(){
		...
		if($this->uid==$JobInfo['uid']){
		    ...
		    if($JobInfo['state']=="0"){
		        ...
		        $this->yunset('stop','1');
				$this->ACT_msg($this->config['sy_weburl'],"该职位已下架！");
		    }
		}
		...
		if($this->usertype=="1"&&$this->uid){
			...
			if($resume!='0'){ 
				...
				if(!empty($look_job)){
					...
					$this->automsg("用户 ".sub_string($member['username'])." 浏览了您的职位：".$JobInfo['name'],$JobInfo['uid']);
				}
			}
		}
		...
		$ComInfo=$UserinfoM->GetUserinfoOne(array('uid'=>$JobInfo['uid']),array('field'=>"`shortname`,`hy`,`pr`,`mun`,`provinceid`,`ant_num`,`linkqq`,`logo`,`logo_status`,`address`,`busstops`,`linktel`,`linkman`,`linkphone`,`email_status`,`moblie_status`,`yyzz_status`,`content`,`x`,`y`,`welfare`,`infostatus`",'usertype'=>2));
