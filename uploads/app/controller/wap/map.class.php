...
class map_controller extends common{
    ...
    function joblist_action(){
        ...
        $pagenav=($page-1)*10;
        $limit="$pagenav,10";
        $where = array('r_status'=>'1','status'=>'0','state'=>'1','x>'=>0,'y>'=>0);
        if ($this->config['did']){
            $where['did']=$this->config['did'];
        }
        $rows=$JobM->GetComjobList($where,array('field'=>$select,'limit'=>$limit,'orderby'=>'distance','desc'=>'asc'));
        if ($rows){
            foreach ($rows as $v){
                $uids[]=$v['uid'];
            }
	...
	function comlist_action(){
	    ...
	    $limit="$pagenav,10";
	    $CompanyM=$this->MODEL('company');
	    $where = array("`name`<>'' and `hy`<>'' and `r_status`<>2 HAVING `distance`<20 ORDER BY `distance` ASC");
	    if ($this->config['did']){
	        $where['did']=$this->config['did'];
	    }
	    $comrows=$CompanyM->GetComList($where,array('field'=>$select,'limit'=>$limit));
	    if($comrows){
	        $comall=$CompanyM->GetComList($where,array('field'=>$select));
