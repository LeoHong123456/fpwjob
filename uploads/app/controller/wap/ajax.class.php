...
class ajax_controller extends common{
	...
	function sava_ajaxresume_action(){
		...
		$JobM=$this->MODEL("job");
		$lt_num=$this->MODEL("lietou")->GetLietoujobNum(array("uid"=>$this->uid,'status'=>1,'zp_status'=>'0','id'=>$data['jobid']));
	...
	function forlink_action(){
		...
		$user=$this->obj->DB_select_alls("resume","resume_expect",
			"a.`r_status`<>'2' and a.`uid`=b.`uid` and b.`id`='".$eid."'","a.name,a.basic_info,a.telphone,a.telhome,a.email,a.uid,b.id,b.`height_status`");
		$user=$user[0];
  ...
  function get_coupon_action(){
	...
	function userreg_action(){
		...
		if(md5(strtolower($_POST['authcode']))!=$_SESSION['authcode']  || empty($_SESSION['authcode'])){
			...
			if($ismoblie>0){
			...
			}elseif(!$row['uname'] || !$row['birthday'] || !$row['exp'] || !$row['edu'] || !$row['telphone']){
				$arr['status']='2';
				$arr['msg']='请填写必要申请信息！';
				...
				if($userid){
					...
					if($this->config['user_state']!="1"){
						$arr['status']='2';
						$arr['msg']='您的账号需要通过审核，才能投递简历哦！';
					}elseif(!$this->config['resume_status']){
						$arr['status']='2';
						$arr['msg']='您的简历需要通过审核，才能投递简历哦！';
					}elseif($this->config['user_sqintegrity'] && $this->config['user_sqintegrity']>$expect['integrity']){
						$arr['status']='3';
						$arr['msg']='该简历完整度未达到'.$this->config['user_sqintegrity'].'%，请先完善简历！';
						$arr['url']= Url('wap',array("c"=>"resume","eid"=>"".$eid.""),'member');
						...
						$notice = $this->MODEL('notice');
						if($this->config['sy_email_set']=="1"){
							if($job_link['email'] && $comjob['is_email']=='1'){
	    ...
	    if($id>0){
	        if($this->uid){
	            ...
	            if(is_array($atninfo)&&$atninfo){
	                $this->obj->DB_delete_all("atn","`uid`='".$this->uid."' AND `sc_uid`='".$id."' AND (`xjhid`=0 OR `xjhid` is NULL)");
    ...
    function atn_academy_action(){
        ...
        if($id>0){
            if($this->uid){
                if($this->usertype=='1'){
                    $academy=$this->obj->DB_select_once("school_academy","`id`='".$id."'","`schoolname`");
                    $name = $academy['schoolname'];
                    $atninfo = $this->obj->DB_select_once("atn","`uid`='".$this->uid."' AND `sc_uid`='".$id."'");
                     if(is_array($atninfo)&&$atninfo){
                         	$this->obj->DB_delete_all("atn","`uid`='".$this->uid."' AND `sc_uid`='".$id."'");
                            $this->obj->member_log("取消了对院校".$name."的关注",5,3);
                            echo "3";die;
                    }else{
                            $this->obj->DB_insert_once("atn","`uid`='".$this->uid."',`sc_uid`='".$id."',`usertype`='".(int)$this->usertype."',`sc_usertype`='5',`time`='".time()."'");  
                             $this->obj->member_log("关注了院校".$name,5,1);
                            echo "4";die;
                    }
                }else{
                    echo "2";die;
	...
	function wap_school_action(){
		...
		if($rows&&is_array($rows)){
			foreach($rows as $v){
				$data.= "<option value='".$v['id']."'>".$v['schoolname']."</option>";
			}
		}
		echo $data;
	}
    ...
    function msgNum_action(){
		...
		if($this->uid){
			...
			if($this->usertype == 1){
				...
				$jobMsgNum=$this->obj->DB_select_num("company_msg","`cuid`='".$this->uid."'and `reply`=''");
				if($jobMsgNum> 0){
					$msgNum += $jobMsgNum;
				}
				...
				$userid_job=$this->obj->DB_select_num("userid_job","`com_id`='".$this->uid."' and `is_browse`='1'");
				if($userid_job > 0){
					$msgNum += $userid_job;
				}
				
				$entrust=$this->obj->DB_select_num("entrust","`lt_uid`='".$this->uid."' and `remind_status`='0'");
				if($entrust > 0){
					$msgNum += $entrust;
				}
				
				$msg=$this->obj->DB_select_num("msg","`job_uid`='".$this->uid."' and `com_remind_status`='0'");
				if($msg > 0){
					$msgNum += $msg;
				}
				...
				$message=$this->obj->DB_select_num("px_zixun","`s_uid`='".$this->uid."' and `status`='1'");
				if($message > 0){
					$msgNum += $message;
				}
				
				$sign_up=$this->obj->DB_select_num("px_baoming","`s_uid`='".$this->uid."' and `status`='0'");
				if($sign_up > 0){
					$msgNum += $sign_up;
				}
