...
class redeem_controller extends common{
	function index_action(){
		...
		if($this->usertype==1){
			...
			$user=$this->obj->DB_select_once("lt_info","`uid`='".$this->uid."'","`photo_big`");
			$photo = $user['photo_big'];
			
		}else if($this->usertype==4){
			$user=$this->obj->DB_select_once("px_train","`uid`='".$this->uid."'","`logo`");
			$photo = $user['logo'];
		}
	...
	function savedh_action(){
		...
		if(!$this->uid && !$this->username){
			...
			$this->obj->member_log("积分兑换，商品ID：".$gfit['id'],17,1);
