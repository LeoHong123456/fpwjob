...
class login_controller extends common{
			...
			if ($this->config['sy_msg_isopen'] && $this->config['sy_msg_login'] && $_POST['act_login']) {
				...
				if($this->config['sy_uc_type']=="uc_center"){
					...
					if($uid > 0) {
						...
						if(!empty($user)){
							...
							if($_SESSION['qq']['openid']){
							    ...
							    $UserinfoM->UpdateMember($qqdata,array("uid"=>$user['uid']));
								unset($_SESSION['qq']);
							}
					...
					if ($this->config['sy_msg_isopen'] && $this->config['sy_msg_login'] && $_POST['act_login']) {
						...
						$userinfo = $UserinfoM->GetMemberOne(array('moblie'=> $moblie),array('field'=>"uid,username,usertype,password,salt,status,did,login_date"));
						...
						if ($cert_arr) {
							...
							if($checkTime){
						 	...
						 	$this->layer_msg('验证码发送不成功，请重新点击发送验证码！'); 
						}
					}else{
						...
						if(!$userinfo&&CheckMoblie($username)){
							$userinfo = $UserinfoM->GetMemberOne(array("`moblie`='".$username."'"),array('field'=>"username,usertype,password,uid,salt,status,did,login_date"));
						...
						if(!$userinfo&&CheckRegEmail($username)){
							$userinfo = $UserinfoM->GetMemberOne(array("`email`='".$username."'"),array('field'=>"username,usertype,password,uid,salt,status,did,login_date"));
					...
					if($res){
						...
						if($_SESSION['qq']['openid']){
						    ...
						    $UserinfoM->UpdateMember($qqdata,array("uid"=>$userinfo['uid']));
							unset($_SESSION['qq']);
						}
						if($_SESSION['wx']['openid']){
							$udate = array('wxid'=>$_SESSION['wx']['openid']);
						...
						if($_SESSION['mag']["user_id"]){
						    ...
						    $UserinfoM->UpdateMember(array("maguid"=>$_SESSION['mag']["user_id"]),array("uid"=>$userinfo['uid']));
						    unset($_SESSION['mag']);
						}
						
						if ($_POST['qfyuid']!=''){
						    $UserinfoM->UpdateMember(array("qfyuid"=>''),array("qfyuid"=>$_POST['qfyuid']));
						    $UserinfoM->UpdateMember(array("qfyuid"=>$_POST['qfyuid']),array("uid"=>$userinfo['uid']));
						}
	...
	function sendmsg_action(){
		if(!$this->config['sy_msg_isopen'] || !$this->config['sy_msg_login']){
			...
			$UserinfoM=$this->MODEL('userinfo');
 			...
 			$binding = $UserinfoM->GetUserinfoOne(array('uid'=>$userinfo['uid'],'moblie_status'=>1),array('usertype'=>$userinfo['usertype'],'field'=>"uid"));
			if(!$binding){
	...
	function qfybind_action(){
	    if ($this->config['sy_qfylogin']==1 && $_POST['qfyuid']!=''){
	        $userinfoM = $this->MODEL('userinfo');
	        $userinfo = $userinfoM->GetMemberOne(array("qfyuid"=>$_POST['qfyuid']),array("field"=>"`uid`,`usertype`,`username`,`email`,`password`,`salt`,`status`,`did`"));
	        $time = time();
