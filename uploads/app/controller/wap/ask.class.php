...
class ask_controller extends common{
	function index_action(){
		...
		$ltinfo=$this->obj->DB_select_all('lt_info','`uid` in('.implode(',',$uids).')','`uid`,`photo`');
	...
	function addquestions_action(){
		...
		if($nid){
			...
			$this->obj->member_log("发布了问答《".$_POST['title']."》",19,1);
			unset($_SESSION['authcode']);		
	...
	function qrepost_action(){
		...
		if(empty($is_set)){
           ...
           $Userinfo=$UserInfoM->GetMemberOne(array('uid'=>$question['uid']),array('field'=>'`username`'));
			...
			if($new_id){
			    $this->obj->member_log('举报问答问题',23,1);
				echo '1';
			}else{
				echo '0';
			}
	...
	function forsupport_action(){
		...
		if(in_array($_POST['aid'],$cookid)){
			...
			if($id){
			    $this->obj->member_log("给问题回答点赞",19,1);
				$sendid=array();
				$sendid[] =$_POST['aid'];
				if($_COOKIE['support']){
