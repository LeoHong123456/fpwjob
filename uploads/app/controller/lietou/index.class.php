...
class index_controller extends lietou_controller{
	function index_action(){
        ...
        if(is_numeric($this->uid)&&(intval($this->uid)>0)){
            $UserJobNumAll=$M->GetLietoujobNum(array('uid'=>$this->uid,"status"=>1," `zp_status`='0' and `r_status`<>'2'"));
            $UserJobNumOverdue=$M->GetLietoujobNum(array('uid'=>$this->uid));
	...
	function post_action(){
		...
		if($_GET['jobid']||$_GET['citys']||$_GET['minsalary']||$_GET['maxsalary']||$_GET['uptime']){
	...
	function recuser_action(){
		...
		if($_POST['submit']){
			...
			if($nid){
				...
				$this->obj->member_log("为悬赏职位推荐人才",25,1);
		...
		$jobinfo['minsalary']=floatval($jobinfo['minsalary']);
		$jobinfo['maxsalary']=floatval($jobinfo['maxsalary']);
	...
	function loginsave_action(){
		...
		if($username && $_POST['password']){
			...
			$userinfo = $Member->GetMemberOne(array('username'=>$username,'usertype'=>3));
			if(!$userinfo && CheckMoblie($username)){
				$userinfo = $Member->GetMemberOne(array('moblie'=>$username,'usertype'=>3));
				$binding=$Member->GetUserinfoOne(array("`uid`=".$userinfo['uid']." and `moblie_status`=1"),array('usertype'=>$userinfo['usertype'],'field'=>"uid"));
				if(!$binding){
					echo json_encode(array('msg'=>'手机号未绑定，不能登录！'));die;
 				}
			}
			if(!$userinfo&&CheckRegEmail($username)){
				$userinfo = $Member->GetMemberOne(array('email'=>$username,'usertype'=>3));
 				
				$binding=$Member->GetUserinfoOne(array("`uid`=".$userinfo['uid']." and `email_status`=1"),array('usertype'=>$userinfo['usertype'],'field'=>"uid"));
				if(!$binding){
					echo json_encode(array('msg'=>'邮箱未绑定，不能登录！'));die;
 				}
			}
