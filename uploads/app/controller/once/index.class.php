...
class index_controller extends common{
	...
	function ajax_action(){
		...
		if($_POST['type']==1){
			...
			$data['url']=Url('once',array('c'=>'add','id'=>(int)$jobinfo['id']));
	...
	function show_action(){
		if(isset($_GET['id'])){
			...
			if($o_info['pic']&&file_exists(str_replace('./',APP_PATH.'/',$o_info['pic']))){
				$o_info['pic']=str_replace('./',$this->config['sy_weburl'].'/',$o_info['pic']);
			}else{
				$o_info['pic']='';
			}
	...
	function wxurl_action(){
		...
		if((int)$_POST['orderId']){
			$order=$this->obj->DB_select_once("company_order","`id`='".(int)$_POST['orderId']."' AND `order_state`='1'");
	...
	function add_action(){
        ...
        $this->yunset($CacheList);
		$M=$this->MODEL('once');
		$info=$M->GetOncejobOne(array('id'=>(int)$_GET['id']));
		if($info){
			if($info['edate']>mktime()){
				$info['edate']=ceil(($info['edate']-mktime())/86400);
			}else{
				$info['edate']="已过期";
			}
			if($info['provinceid']){
				$info['provincename']=$CacheList['city_name'][$info['provinceid']];
			}
			if($info['cityid']){
				$info['cityname']=$CacheList['city_name'][$info['cityid']];
			}
			if($info['three_cityid']){
				$info['three_cityname']=$CacheList['city_name'][$info['three_cityid']];
			}
		}
		if(!$info['pic']||!file_exists(APP_PATH.'/'.$info['pic'])){
			if($this->config['sy_once_icon']){
				$info['pic_n']= $this->config['sy_weburl']."/".$this->config['sy_once_icon'];
			}
		}else{
			$info['pic_n']= $this->config['sy_weburl']."/".$info['pic'];
		}
