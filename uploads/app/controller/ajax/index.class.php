...
class index_controller extends common{
	...
	function sava_ajaxresume_action(){
        ...
        $JobM=$this->MODEL("job");
			$lt_num=$this->MODEL("lietou")->GetLietoujobNum(array("uid"=>$this->uid,"`r_status`<>'2'",'status'=>1,'zp_status'=>'0','id'=>$data['jobid']));
	...
	function for_link_action(){
		...
		if ($this->config['com_lietou_job']=='1'){
			...
			}elseif($this->usertype=='2' && $user['height_status']=='2'){
				$ltjob=$this->MODEL("lietou");
				$list=$ltjob->GetLietoujobOne(array("uid"=>$this->uid,"status"=>1,"`r_status`<>'2'"),array("field"=>"`id`,`job_name`"));
				if(empty($list)){
					$arr['msg']="未发布高端职位无法查看下载简历！";
					$arr['status'] = 1;
					$this->_out($arr);
				}
			}
			...
			if($row['rating_type']=='1'){
				...
				if($row['down_resume']=='0'){
					if($this->config['com_integral_online']!="4"){
						...
						$arr['status']=7;
						$arr['msg']="你的套餐已经用完，你可以<a href='".$this->config['sy_weburl']."/member/index.php?c=right' style='color:red;cursor:pointer;'>购买会员</a>！";
					}
	...
	function lt_down_resume_action(){
		...
		if($type=="integral"){
			if($row["integral"]<$this->config['integral_lt_downresume'] && $this->config['integral_lt_downresume_type']=="2"){
				...
				$this->obj->member_log("下载了 ".$username['username']." 的高级简历。",3,1);
				$this->addstate($state_content,2);
				$Warning=$this->MODEL("warning");
				$Warning->warning("2");
				$arr['status']=3;
	...
	function entrust_action(){
		...
		if(is_array($row)){
			...
			if(is_array($resume)){
				...
				$this->obj->member_log("把高级简历 《".$resume['name']." 》委托给：".$_POST['name'],6,1);
				echo 3;die;
			}else{
				echo 4;die;
			}
	    ...
	    if($id>0){
	        if($this->uid){
	            ...
	            if ($tid){
	                ...
	                $atninfo = $this->obj->DB_select_once("atn","`uid`='".$this->uid."' AND `sc_uid`='".$id."' AND (`xjhid`=0 OR `xjhid` is NULL)");
	            ...
	            if(is_array($atninfo)&&$atninfo){
	                if ($tid){
	                    $this->obj->DB_delete_all("atn","`uid`='".$this->uid."' AND `sc_uid`='".$tid."' and `tid`='".$id."' AND (`xjhid`=0 OR `xjhid` is NULL)");
	                ...
	                if ($tid){
	                    ...
	                    $this->obj->DB_insert_once("atn","`uid`='".$this->uid."',`sc_uid`='".$id."',`usertype`='".(int)$this->usertype."',`sc_usertype`='2',`time`='".time()."',`xjhid`=0");
	                    $this->obj->DB_update_all('company',"`ant_num`=`ant_num`+1","`uid`='".$id."'");
	                    $content="关注了<a href=\"".$comurl."\" target=\"_bank\">".$name."</a>";
	                    $this->addstate($content,2);
	                    $msg_content = "用户 ".sub_string($this->username)." 关注了你！";
	...
	function atn_train_action(){
	    ...
	    if($uid>0){
	        if($this->uid){
	            ...
	            if ($uid){
	                $atninfo = $this->obj->DB_select_once("atn","`uid`='".$this->uid."' AND `sc_uid`='".$uid."' and `tid`=''");
	                $train=$this->obj->DB_select_once("px_train","`uid`='".$uid."'","`name`");
	                $name = $train['name'];
	            }
	            if(is_array($atninfo)&&$atninfo){
	                if ($uid){
	                    $this->obj->DB_delete_all("atn","`uid`='".$this->uid."' AND `sc_uid`='".$uid."'  and `tid`=''");	
						$this->obj->DB_update_all('px_train',"`ant_num`=`ant_num`-1","`uid`='".$uid."'");               
	                }
	                $this->obj->member_log("取消了对".$name."关注",5,3);
	                echo "2";die;
	            }else{
	                if ($uid){
	                   $this->obj->DB_insert_once("atn","`uid`='".$this->uid."',`sc_uid`='".$uid."',`usertype`='".(int)$this->usertype."',`sc_usertype`='4',`time`='".time()."'");
	                   $this->obj->DB_update_all('px_train',"`ant_num`=`ant_num`+1","`uid`='".$uid."'");
	                }
	                $this->obj->member_log("关注了".$name,5,1);
	                echo "1";die;
		...
		if($this->uid!=""&&$this->username!=""){
			if($_COOKIE['remind_num']>0){
				...
				if($this->usertype==1){
					...
					$html.='<div class="header_Remind_list_a"><a href="'.$this->config['sy_weburl'].'/member/index.php?c=sign_up"class="fl">课程预约</a><span class="header_Remind_list_r fr">('.$_COOKIE['sign_up'].')</span></div><div class="header_Remind_list_a"><a href="'.$this->config['sy_weburl'].'/member/index.php?c=sysnews"class="fl"> 系统消息</a><span class="header_Remind_list_r fr">('.$_COOKIE['sysmsg4'].')</span></div><div class="header_Remind_list_a"><a href="'.$this->config['sy_weburl'].'/member/index.php?c=message"class="fl">咨询留言</a><span class="header_Remind_list_r fr">('.$_COOKIE['message'].')</span></div>';
				}
				$html.='</div> </div>';
			}
			...
			$login_train_url = Url("train",array("c"=>"login"));
			...
			if($_GET['f']=='l'){
				...
				$kjlogin = '';
				$html='<div class=" fr"><div class="yun_topLogin_cont"><div class="yun_topLogin"><a class="yun_More" href="javascript:void(0)">用户登录</a><ul class="yun_Moredown" style="display:none">'.$login.$lt_login.$train_login.'{kjlogin}</ul></div><div class="yun_topLogin"> <a class="" href="'.Url("register").'">免费注册</a></div></div></div>';
	...
	function DefaultLoginIndex_action(){
		if($this->usertype=='1' && $this->uid){
			...
			$lt[0]['lt_status2']=$this->obj->DB_select_num("down_resume","`uid`='".$this->uid."'");
	...
	function LoginHead_action(){
		if($this->uid!=""&&$this->username!=""){
			...
			$reg_train_url = Url('train',array("c"=>"register"));
			$style = $this->config['sy_weburl']."/app/template/".$this->config['style'];
			$html.='<div class="hp_top_rt_login fl">你好，请<a class="hp_top_rt_login_g" href="'.$log_url.'">登录</a></div><i class="hp_top_line fl"> | </i><div class="hp_top_rt_regist fl"><a class="hp_top_rt_regist_m" href="javascript:void(0);">免费注册 <i class="hp_top_rg_down"></i></a><div class="hp_top_regist_list" style="display:none;"><ul><li><a href="'.$reg_url.'">个人注册</a></li><li><a href="'.$reg_com_url.'">企业注册</a></li><li><a href="'.$reg_lt_url.'">猎头注册</a></li><li><a href="'.$reg_train_url.'">培训注册</a></li></ul></div></div>';
			echo "document.write('".$html."');";
		}
	}
	    ...
	    if ($pic!=''){
	        ...
	        if ($this->usertype == 3){
	            ...
	            if ($img =='logo'){
	                ...
	                if ($nid){
	                    $this->obj->member_log("上传猎头头像",16,1);
	                }
	            }
	        }
	        if ($this->usertype == 4){
	            ...
	            if ($img =='logo'){
	                $train = $trainM->GetTrainInfo(array('uid'=>$this->uid),array('field'=>'logo'));
	                if($train['logo']){
	                    unlink_pic($train['logo']);
	                }else{
	                    $IntegralM=$this->MODEL('integral');
	                    $IntegralM->get_integral_action($this->uid,"integral_avatar","上传LOGO");
	                }
	                $nid = $trainM->UpdateTrain(array('logo'=>$pic),array('uid'=>$this->uid));
	                if ($nid){
	                    $this->obj->member_log("上传培训机构LOGO",16,1);
	                }
	            }
 	     }
	    }
	...
	function get_school_option_action(){
		if($_POST['type']=='provinceid'&&$_POST['cityid']){
			$where="`provinceid`='".$_POST['cityid']."'";
		}elseif($_POST['type']=='cityid'&&$_POST['cityid']){
			$where="`cityid`='".$_POST['cityid']."'";
		}else{
			$where=1;
		}
		$rows=$this->obj->DB_select_all('school_academy',$where,'`id`,`schoolname`');
		$html = '<option value="">请选择</option>';
		if($rows&&is_array($rows)){
			foreach($rows as $v){
				$html .= "<option value='".$v['id']."'>".$v['schoolname']."</option>";
			}
		}
		echo $html;die;
	}
