...
class index_controller extends common{
	...
	function public_action(){
		...
		$ComMember=$UserinfoM->GetMemberOne(array("uid"=>(int)$_GET['id']),array("field"=>"`source`,`email`,`claim`,`status`,`login_date`"));
		$this->yunset("ComMember",$ComMember);
		 
  		$num=$JobM->GetComjobNum(array('uid'=>(int)$_GET['id'],'r_status'=>1,'status'=>'0','state'=>1));
		$this->yunset("num",$num);
		...
		$comrat=$UserinfoM->GetRatinginfoOne(array("id"=>$userstatis['rating']));
        ...
        if(!$row['logo'] || $row['logo_status']!='0' || !file_exists(str_replace("./",APP_PATH,$row['logo']))){
		...
		if($this->config['com_login_link']=='2'){
		    ...
		    $look_msg="企业没有公开联系信息！";
		    $looktype="3";
		}elseif($this->config['com_login_link']=='1' || $_GET['id']==$this->uid){
			$looktype="1";
    ...
    function show_action(){
		...
		$xjh = $this->obj->DB_select_all("school_xjh","`status`=1 and `uid`='".$_GET['id']."' limit=3");
		$this->yunset('xjh',$xjh);
