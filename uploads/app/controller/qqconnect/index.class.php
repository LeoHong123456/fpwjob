	...
	function index_action(){
	    ...
	    if(empty($code)){
	     ...
	     if($user->openid!=""){
	        ...
	        if ($user->unionid){
	            $qqwhere.=" OR (`qqunionid`='".$user->unionid."' AND `qqunionid`<>'')";
	        }
	        $userinfo = $this->obj->DB_select_once("member",$qqwhere);
			if(is_array($userinfo)){
			    $qqdata=array('login_date'=>time());
			    if ($user->unionid){
			        $qqdata['qqunionid']=$user->unionid;
			    }
			    $this->obj->update_once('member',$qqdata,array('uid'=>$userinfo['uid']));
				...
				if($this->uid){
				    $this->obj->DB_update_all("member","`qqid`='',`qqunionid`","`qqid`='".$_SESSION['qq']["openid"]."' OR (`qqunionid`='".$_SESSION['qq']["unionid"]."' AND `qqunionid`<>'')");
					$this->obj->DB_update_all("member","`qqid`='".$_SESSION['qq']["openid"]."',`qqunionid`='".$_SESSION['qq']["unionid"]."'","`uid`='".$this->uid."'");
    ...
    function cert_action(){
		...
		if($id && is_array($arr) && $arr[0] && $arr[2]==$this->config['coding']){
			...
			if(is_array($row)){
				...
				$this->obj->DB_update_all("px_train","`email_status`='0',`linkmail`=''","`linkmail`='".$row['check']."'");
