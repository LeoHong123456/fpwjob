...
class usercert_controller extends siteadmin_controller{ 
	...
	function status_action(){
		...
		if($this->config['sy_email_usercert']=='1'){
			$userinfo = $UserinfoM->GetMemberOne(array('uid'=>$uid),array('field'=>"`email`,`name`,`username`,`uid`,`usertype`"));
      ...
      $notice->sendEmailType(array("email"=>$userinfo['email'],'uid'=>$data['uid'],'name'=>$data['name'],"certinfo"=>$statusbody,"username"=>$userinfo['username'],"type"=>"usercert"));
		...
		if($id){
			if($status==1){
				$com=$UserinfoM->GetCompanyPayOne(array("com_id"=>$uid,"pay_remark"=>"上传身份验证"),array("field"=>"`com_id`"));
				if(empty($com)){
	...
	function idcard_status_action(){
		...
		if($this->config['sy_email_usercert']=='1'){
			$userinfo = $M->GetMemberList(array("`uid` IN (".$_POST['allid'].")"),array("field"=>"`email`,`name`,`username`,`uid`,`usertype`"));
