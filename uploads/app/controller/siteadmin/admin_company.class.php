...
class admin_company_controller extends siteadmin_controller{
	...
	function add_action(){
		...
		if($_POST['submit']){
			...
			if($username==""||mb_strlen($username)<2||mb_strlen($username)>16){
				...
				$data['type']='8';
			}elseif($password==""||mb_strlen($password)<2||mb_strlen($password)>20){
				...
				if(is_array($user)){
					...
					if($uid>0){
						...
						$ratingM = $this->MODEL('rating');
						$value.=$ratingM->rating_info($rat_arr[0],$uid);
	...
	function mjob_action(){
	    ...
	    $urlarr['page']="{{page}}";
	    $pageurl=Url($_GET['m'],$urlarr,'admin');
	    $rows=$this->get_page("company_job",'`uid`='.intval($_GET['comid']).'',$pageurl,$this->config['sy_listnum']);
	...
	function morder_action(){
	    ...
	    $search_list[]=array("param"=>"typedd","name"=>'订单类型',"value"=>array("1"=>"会员充值","2"=>"".$this->config['integral_pricename']."充值","3"=>"银行转帐","4"=>"金额充值","5"=>"购买增值包","6"=>"课程订购","7"=>"购买小程序",'8'=>'分享职位赏金','9'=>'悬赏职位赏金',"26"=>"购买广告位"));
