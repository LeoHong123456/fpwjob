...
* 
 */
class member_log_controller extends siteadmin_controller
{
	function index_action(){
		$where=1;
		if((int)$_GET['type']){
			$where.=" and `usertype`='".(int)$_GET['type']."'";
			$urlarr['type']=(int)$_GET['type'];
		}else{
			$where.=" and `usertype`='1'";
		}
		if((int)$_GET['operas']){
			$where.=" and `opera`='".(int)$_GET['operas']."'";
			$urlarr['operas']=(int)$_GET['operas'];
		}
		if((int)$_GET['parr']){
			$where.=" and `type`='".(int)$_GET['parr']."'";
			$urlarr['parr']=(int)$_GET['parr'];
		}
		if($_GET['end']){
			if($_GET['end']=='1'){
		...
		$this->siteadmin_tpl(array('member_log'));
	}
