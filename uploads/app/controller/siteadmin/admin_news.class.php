...
class admin_news_controller extends siteadmin_controller
{
	...
	function property_action(){
		if(!$_POST['id']){
			...
			isset($nbid)?$this->ACT_layer_msg( "新闻属性(ID:".$nbid.")添加成功！",9):$this->ACT_layer_msg( "添加失败！",8);
		}else{
			$nbid=$this->obj->DB_update_all("property","`name`='".$_POST['name']."',`value`='".$_POST['value']."'","`id`='".$_POST['id']."'");
			isset($nbid)?$this->ACT_layer_msg( "新闻属性(ID:".$_POST['id'].")更新成功！",9):$this->ACT_layer_msg( "更新失败！",8);
		}
	}
	...
	function addnews_action()
	{
	    ...
	    if($_POST['newsadd']){
			if($_FILES['uploadpic']['tmp_name']){
 				$upload=$UploadM->Upload_pic("../data/upload/news/",false);
				$pic=$upload->picture($_FILES['uploadpic']);
				$picmsg=$UploadM->picmsg($pic,$_SERVER['HTTP_REFERER']);
 				if($picmsg['status'] == $pic){
					$this->ACT_layer_msg($picmsg['msg'],8);
				}
				$s_thumb=$upload->news_makeThumb($pic,120,120,'_S_');
				$value.="`newsphoto`='" . str_replace("../data/upload/new","data/upload/new",$pic) . "',";
	...
	function group_action(){
      ...
      if(is_array($countid)){
		...
		$this->siteadmin_tpl(array('admin_news_group'));
	}
