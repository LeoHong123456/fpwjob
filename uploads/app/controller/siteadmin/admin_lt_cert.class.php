...
class admin_lt_cert_controller extends siteadmin_controller{
	...
	function index_action(){
		...
		if(trim($_GET['keyword'])){
			$where.=" and b.`realname` like '%".trim($_GET['keyword'])."%'";
		...
		if($_GET['order']){
			...
			$where.=" order by a.status asc,a.`id` desc";
		}
		...
		$count=$this->obj->DB_select_alls("company_cert","lt_info","a.`uid`=b.`uid` and a.`type`='4' $where","a.uid");
 		$count?$num = count($count):$num=0;
