 ...
 */
class invite_controller extends siteadmin_controller
{
	function set_search(){
	...
	function index_action()
	{
		...
		if(trim($_GET['keyword'])){
			if($_GET['type']=="1"){
				$info=$UserInfoM->GetMemberList(array("`username` like '%".trim($_GET['keyword'])."%'"),array("field"=>"uid,username","special"=>'1'));
				if(is_array($info)){
					foreach ($info as $v){
						$uid[]=$v['uid'];
					}
		...
		$M=$this->MODEL();
		$rows=$M->get_page("userid_msg",$where,$pageurl,$this->config['sy_listnum']);
		if(is_array($rows['rows'])&&$rows['rows']){
			if(trim($_GET['keyword'])==''||$_GET['type']!='1'){
				$uid=array();
				foreach($rows['rows'] as $v){
					$uid[]=$v['uid'];
				}
				$info=$UserInfoM->GetMemberList(array("`uid` in (".@implode(",",$uid).")"),array("field"=>"uid,username","special"=>'member'));
			}
		...
		$this->yunset($rows);
		$this->siteadmin_tpl(array('invite'));
	}
	function del_action() {
		$this->check_token();
	    if($_GET['del']){
			...
			$CompanyM->DeleteUserMsg(array("`id` in (".$del.")"));
