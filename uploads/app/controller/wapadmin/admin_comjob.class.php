...
class admin_comjob_controller extends adminCommon{
	function index_action(){ 
        ...
        $rows=$this->get_page("company_job",$where." order by state asc,lastupdate desc",$pageurl,$this->config['sy_listnum']);
			    ...
			    if($data!=""){
			        ...
			        if ($push){
			            if ($data['clientid']&& $data['clientid']!=''){
			                $puser = array('clientid'=>$data['clientid'],'deviceToken'=>$data['deviceToken']);
			                if ($data['type']=='zzshtg'){
			                    $content = mb_substr('职位：'.$data['jobname'].'审核通过', 0, 24, 'UTF-8');
			                }elseif ($data['type']=='zzshwtg'){
			                    $content = mb_substr('职位：'.$data['jobname'].'审核未通过', 0, 24, 'UTF-8');
			                }
