...
function reg_checkAjax(id) {
	...
	if(id == "email1") {
		...
		if(obj == "") {
			...
			$.post(weburl + "/index.php?m=register&c=regemail", {
				email: obj
			}, function(data) {
				if(data == 0) {
					msg = "填写正确！"
...
function checkform(img) {
	...
	if($("#username").attr('date') != "1" || $("#regpassword").attr('date') != "1" || $("#passconfirm").attr('date') != "1" || $("#email1").attr('date') != "1" || $("#moblie").attr('date') != "1" || (exitsid("realname") && $("#realname").attr('date')!="1")) {
		return false;
	} else {
		...
		if($("#xieyi").attr("checked") != 'checked') {
			layer.msg('您必须同意注册协议才能成为本站会员！', 2, 8);
			return false;
		} else {
	...
	if(date == 1 && send == 0) {
		...
		$.post(weburl + "/index.php?m=ajax&c=regcode", {
			...
			if(data == 0) {
				...
				layer.msg('图片验证码错误！', 2, 8, function() {
					checkCode(img);
				});
				return false;
			} else if(data == 6) {
