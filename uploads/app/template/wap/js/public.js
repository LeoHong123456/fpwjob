...
$(document).ready(function () {
	...
	$(".sq_resume").click(function(){
		...
		$.post(wapurl+"/index.php?c=ajax&a=indexajaxresume",{show_job:1,jobid:jobid,jobtype:jobtype},function(data){
			...
			if(status == '7'){
				...
				layermsg('未审核企业用户不能邀请面试！请联系客服加快审核进度', 2, 8);return false;
			}else if(status == '4'){
	...
	$("#click_invite").click(function(){
		...
		layer_load('执行中，请稍候...');
		$.post(wapurl+"/index.php?c=ajax&a=sava_ajaxresume",{uid:uid,content:content,username:username,jobname:jobname,address:address,linkman:linkman,linktel:linktel,intertime:intertime,jobid:jobid},
			function(data){
			layer.closeAll();
			var data=eval('('+data+')');
			var status=data.status;
...
function checkanswer(uid,img){
	...
	$.post(wapurl+"/index.php?c=ask&a=answer",{id:id,content:content,authcode:authcode},function(data){
		if(data=='0'){
			...
			layermsg('发布回答成功！',2,function(){window.location.reload(true);});return false; 
		}else if(data==2){
...
function atn(id,url){
	if(id){
		$.post(url,{id:id},function(data){
			if(data==1){
                layermsg('关注企业成功！',2,function(){window.location.reload(true);});return false; 
             
			}else if(data==2){
                layermsg('取消关注企业成功！',2,function(){window.location.reload(true);});return false; 
	...
	if(id){
		$.post(wapurl+'index.php?c=ajax&a=atn_teacher',{id:id,tid:tid},function(data){
			if(data==1){
				...
				layermsg('请先登录！只有个人、企业、猎头用户才能关注！');return false;
			}else if(data==4){
				layermsg('培训用户不能关注讲师！');return false;
			}
		});
	}
}
...
function istrainlogin(){
	layer.open({
		content:"只有培训用户可发布，是否登录培训用户？",
		btn: ['登录', '取消'],
		shadeClose: false,
		yes: function(){
			window.location.href ='index.php?c=login';window.event.returnValue = false;return false;
		}
	});
}
