...
function invite() {
	...
	if(linktel == '') {
		layermsg('请填写联系人电话！', 2, 8);
		return false;
	}
	if(intertime == '') {
		layermsg('请选择面试日期！', 2, 8);
		return false;
	}
	if(address == '') {
		layermsg('请填写面试地址！', 2, 8);
		return false;
	}
	$.post('index.php?c=logstatus', {
		rewardid: rewardid,
...
function withdraw_form() {
	...
	if(!real_name) {
		layermsg('请填写真实姓名！', 2, 8);
		return false;
	}
	if(Number(price) < 1) {
		layermsg('请正确填写提现金额！', 2, 8);
		return false;
	}
...
function change_form(){
	...
	if(changeprice == ''){
		layermsg('请正确填写转换金额！',2,8);return false;
	}
	return true;
}
