...
if(!empty($_POST)){
	...
	if($_SESSION['xsstooken']!=sha1($config['sy_safekey']) || $config['sy_safekey']=='')
	{
		...
		foreach($_POST  as $id=>$v){
		    if(!in_array($id,array('preview','uimage','safekey','token','deviceToken','notify_data'))){
		...
		foreach($_POST  as $id=>$v){
			if (preg_match('/^[_0-9a-z]{1,30}$/i',$id)){
				foreach($arr as $arrkey=>$arrv){
					if(!in_array($id,array('content','config','group_power','description','body','job_desc','eligible','other','code','intro','doc','traffic','media','packages','booth','participate','expinfo','eduinfo','skillinfo','projectinfo'))){
