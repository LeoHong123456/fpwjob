...
class admin_trust_controller extends adminCommon{
	...
	function status_action(){
		...
		if($status=='2'){
			...
			if($user_entrust){ 
 				$IntegralM->company_invtal($user_entrust['uid'],$user_entrust['price'],true,"退还委托简历费用",true,2,'packpay');
			}
			$this->automsg("委托简历未通过审核",$user_entrust['uid']);
		}else if ($status=='1') {
			$this->obj->DB_update_all("resume_expect","`is_entrust`='2'","`uid`='".$user_entrust['uid']."' and `id`='".$user_entrust['eid']."'");
			$this->automsg("委托简历通过审核",$user_entrust['uid']);
		}else{
			$this->obj->DB_update_all("resume_expect","`is_entrust`='1'","`uid`='".$user_entrust['uid']."' and `id`='".$user_entrust['eid']."'");
		}
	...
	function recom_action(){
		...
		$urlarr['c']='recom';
		$row=$this->obj->DB_select_once("resume_expect","`id`='".$_GET['eid']."'","`id`,`hy`,`city_classid`,`job_classid`");
		...
		if($row['job_classid']){ 
			$where.=" and ( `job1` in(".pylode(',', $row['job_classid']).") or `job1_son` in(".pylode(',', $row['job_classid']).") or `job_post` in(".pylode(',', $row['job_classid']).") )";
 		}	

		if($row['city_classid']){ 
			$where.=" and ( `provinceid` in (".pylode(',',$row['city_classid']).") or `cityid` in (".pylode(',', $row['city_classid']).") or `three_cityid` in (".pylode(',', $row['city_classid']).") )";
 		}
		...
		$rows=$this->get_page("company_job",$where,$pageurl,$this->config['sy_listnum'],"`uid`,`name`,`hy`,`job1`,`job1_son`,`provinceid`,`cityid`,`job_post`,`id`,`minsalary`,`maxsalary`");
		if($rows&&is_array($rows)){
			foreach($rows as $val){
				$uids[]=$val['uid'];
			}
			$company=$this->obj->DB_select_all("company","`uid` in(".@implode(',',$uids).")","`uid`,`name`,`linkmail`");
	...
	function directrecom_action(){ 
 		if($_GET['eid']&&$_GET['jobid']){
			...
			$resume=$this->obj->DB_select_once("resume_expect","`id`='".$_GET['eid']."'","`uid`,`name`");
			...
			if($id){
				...
				$arr['msg']='推荐成功！';
				$arr['type']='9';
				$this->automsg("管理员操作：为您推荐了简历《".$resume['name']."》",$_GET['comid']);
			}else{
				$arr['msg']='推荐失败';
				$arr['type']='8';
			}
