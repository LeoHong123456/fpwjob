	...
	function archive_action(){
		...
		if($_POST['limit']){
			...
			if($type=='job'||$type=='sitemap'){
				...
				$rows['job']=$this->obj->DB_select_all("company_job","`sdate`<'".time()."' and `state`='1' order by ".$order." desc limit ".intval($_POST['limit']),"`id`,`".$order."` as `time`");
			}
			if($type=='resume'||$type=='sitemap'){
				if($_POST['order']=='uptime'){
					$order='lastupdate';
				}else{$order='addtime';}
				$rows['resume']=$this->obj->DB_select_all("resume_expect","`status`<>'2' and `r_status`<>'2' and `job_classid`<>'' and `open`='1'  ORDER BY `".$order."` desc limit ".intval($_POST['limit']),"`id`,`".$order."` as `time`");
			}
