...
class company_order_controller extends adminCommon{
	...
	function set_search(){
		$typedd = array(1 => '会员充值（购买会员）',2 => '积分充值',3 => '银行转帐',4 => '金额充值',5 => '购买增值包',6=>"课程订购",7 => '购买小程序',8 => '分享红包推广',9 => '悬赏红包',10 => '职位置顶',11 => '职位紧急',12 => '职位推荐',13 => '自动刷新',14 => '简历置顶',15 => '委托简历',16 => '刷新职位',17 => '刷新兼职',18 => '刷新猎头职位',19 => '下载简历',20 => '发布职位',21 => '发布兼职',22 => '发布猎头职位',23 => '面试邀请',24 => '兼职推荐',25 => '店铺招聘',26 => '购买广告位');
	...
	function index_action(){
		...
		if(intval($_GET['comid'])){
			$where.=" and `uid`='".intval($_GET['comid'])."'";
	...
	function del_action(){
	    ...
	    if($_GET['del']){
	    	...
	    	if(is_array($del)){
				...
				if($company_order&&is_array($company_order)){
					...
					$this->obj->DB_update_all("user_log","`status`=4","`orderid` in(".@implode(',',$order_ids).")");
	    ...
	    if(isset($_GET['id'])){
			...
			$this->obj->DB_update_all("user_log","`status`=4","`orderid`='".$company_order['order_id']."'" );
