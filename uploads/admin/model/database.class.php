	...
	function clear_action(){
		...
		if($_GET){
			...
			$this->check_token();
 			...
 			$clearData=@implode(',',$_GET['clear_data']);
  
  			if($clearData==""){

				$this->ACT_layer_msg("请选择需要清理的数据！",8);

			}else if($_GET['clear_time']==""){

				$this->ACT_layer_msg("请选择清理时间！",8);

			}else{

				$clearTime = strtotime("-".$_GET['clear_time']." day");
			 	
			 	if(in_array("userid_job",$_GET['clear_data'])){
			 		$nid = $this->obj->DB_delete_all("userid_job","`datetime`<'".$clearTime."'","");
 			 	}
			 	
			 	if(in_array("userid_msg",$_GET['clear_data'])){
			 		$nid = $this->obj->DB_delete_all("userid_msg","`datetime`<'".$clearTime."'","");
 			 	}
 			 	
			 	if(in_array("down_resume",$_GET['clear_data'])){
			 		$nid = $this->obj->DB_delete_all("down_resume","`downtime`<'".$clearTime."'","");
 			 	}
 			 	
			 	if(in_array("talent_pool",$_GET['clear_data'])){
			 		$nid = $this->obj->DB_delete_all("talent_pool","`ctime`<'".$clearTime."'","");
 			 	}
			 	
			 	if(in_array("look_resume",$_GET['clear_data'])){
			 		$nid = $this->obj->DB_delete_all("look_resume","`datetime`<'".$clearTime."'","");
 			 	}
			 	
			 	if(in_array("look_job",$_GET['clear_data'])){
			 		$nid = $this->obj->DB_delete_all("look_job","`datetime`<'".$clearTime."'","");
			 	}
			 	 
			 	if(in_array("email_msg", $_GET['clear_data'])){
			 		$nid = $this->obj->DB_delete_all("email_msg","`ctime`<'".$clearTime."'","");
 			 	}
 		
 			 	if(in_array("moblie_msg", $_GET['clear_data'])){
			 		$nid = $this->obj->DB_delete_all("moblie_msg","`ctime`<'".$clearTime."'","");
 			 	}
 			 	
			 	if(in_array("member_log", $_GET['clear_data'])){
			 		$nid = $this->obj->DB_delete_all("member_log","`ctime`<'".$clearTime."'","","","1");
 			 	}
 			 	
			 	if(in_array("admin_log",$_GET['clear_data'])){
			 		$nid = $this->obj->DB_delete_all("admin_log","`ctime`<'".$clearTime."'","");
 			 	}
 			 	
 			 	if(in_array("recycle", $_GET['clear_data'])){
			 	...
			 	if($nid){
			 		$this->layer_msg('数据清理成功！',9,1,"index.php?m=database&c=clearList");
			 	}else{
			 		$this->layer_msg('数据清理失败！',8,0,"index.php?m=database&c=clearList");
			 	}
 			}
		}
