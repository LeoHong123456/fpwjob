...
class admin_partjob_controller extends adminCommon{
	...
	function index_action(){
        ...
        $rows=$this->get_page("partjob",$where,$pageurl,$this->config['sy_listnum']);
		if(is_array($rows)){
			foreach($rows as $v){
				$jid[]=$v['id'];
			}
	...
	function recommend_action()
	{
		...
		if($pid){
			$info=$this->obj->DB_select_once("partjob","`id`='".$pid."'","`uid`,`name`");
			if($s==1){
				$this->obj->DB_update_all("partjob","`rec_time`='0'","`id`='".$pid."'");
				$this->automsg('管理员操作：取消兼职职位《'.$info['name'].'》推荐',$info['uid']);
			}elseif($eid>time()){
				$this->obj->DB_update_all("partjob","`rec_time`=`rec_time`+$addtime","`id`='".$pid."'");
				$this->automsg('管理员设置：兼职职位《'.$info['name'].'》推荐'.$addday.'天',$info['uid']);
			}else{
		...
		if(!empty($codearr)){
			if($s==1){
				...
				$info=$this->obj->DB_select_all("partjob","`id` in (".$codearr.")","`uid`,`name`");
				if($info){
					foreach($info as $v){
						$this->automsg('管理员操作：取消兼职职位《'.$v['name'].'》推荐',$v['uid']);
					}
				}
				$this->ACT_layer_msg("取消职位推荐设置成功！",9,$_SERVER['HTTP_REFERER'],2,1);
			}else{
				$list=$this->obj->DB_select_all("partjob","`id` in (".$codearr.")","`id`,`rec_time`,`uid`,`name`");
                if(is_array($list)){
                	foreach($list as $v){
                        if($v['rec_time']<time()){
                       	    $gid[]=$v['id'];   
					...
					if($list){
						foreach($list as $v){
							$this->automsg('管理员设置：兼职职位《'.$v['name'].'》推荐'.$addday.'天',$v['uid']);
						}
					}
	...
	function ctime_action(){
		...
		if(is_array($id)){
			...
			$rows=$this->obj->DB_select_all("partjob","`id` in(".pylode(',',$jobid).")","`id`,`state`,`edate`,`uid`,`name`");
			foreach($rows as $value){
				if($value['edate']!=0){
					...
					$this->automsg('管理员操作：兼职职位《'.$value['name'].'》延期'.intval($endtime).'天',$value['uid']);
				}
			}
				...
				foreach($data as $key=>$value){
                    ...
                    if ($push){
                        if ($value['clientid']&& $value['clientid']!=''){
                            ...
                            if ($value['type']=='partshtg'){
                                $content = mb_substr('兼职：'.$value['jobname'].'审核通过', 0, 24, 'UTF-8');
                            }elseif ($value['type']=='partshwtg'){
                                $content = mb_substr('兼职：'.$value['jobname'].'审核未通过', 0, 24, 'UTF-8');
                            }
	...
	function shjobmsg($jobid,$yesid,$statusbody){
		...
		if ($data['type']=='partshtg'){
			$contentmsg = mb_substr('兼职职位：'.$comarr['name'].'审核通过', 0, 24, 'UTF-8');
		}elseif ($data['type']=='partshwtg'){
			$contentmsg = mb_substr('兼职职位：'.$comarr['name'].'审核未通过', 0, 24, 'UTF-8');
		}
		if($contentmsg){
	...
	function del_action()
	{
	    ...
	    if($_GET['del']||$_GET['id']){
			...
			$list=$this->obj->DB_select_all("partjob","`id` in (".$del.")","`uid`,`name`");
			...
			if($list){
				foreach($list as $v){
					$this->automsg('管理员操作：删除兼职职位《'.$v['name'].'》',$v['uid']);
				}
			}
	...
	function refresh_action(){
		...
		$list=$this->obj->DB_select_all("partjob","`id` in (".$_POST['ids'].")","`uid`,`name`");
		if($list){
			foreach($list as $v){
				$this->automsg('管理员操作：刷新兼职职位《'.$v['name'].'》',$v['uid']);
			}
		}
