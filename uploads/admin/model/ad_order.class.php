...
class ad_order_controller extends adminCommon{
	...
	function index_action(){
		...
		$rows=$this->get_page("ad_order",$where,$pageurl,$this->config['sy_listnum']);
	...
	function status_action(){
		...
		if($status=="1"){
			...
			$content="广告订单（ID:".$row['order_id']."）通过审核";
		}else if($status=="2"){
			$IntegralM=$this->MODEL('integral');
			if($row['buytype']=="2"){
				if($row['order_state']==2){
					$IntegralM->company_invtal($row['comid'],$row['price'],true,"广告订单未通过审核，退还现金",true,2,'packpay');
				}
			}elseif($row['buytype']=="1"){
				$IntegralM->company_invtal($row['comid'],$row['integral'],true,"广告订单未通过审核，退还".$this->config['integral_pricename'],true,2,'integral');
			}
			$content="广告订单（ID:".$row['order_id']."）未通过审核，已退还费用";
		}
		if($content){
			$this->automsg($content,$row['uid']);
		}
	...
	function del_action(){
	    ...
	    if($_GET['del']){
	    	...
	    	if($del){
	    		if(is_array($del)){
			    	$pic_url=$this->obj->DB_select_all("ad_order","`id` in(".@implode(',',$del).") and `status`<>0","`pic_url`");
					foreach($pic_url as $val){
						unlink_pic($val['pic_url']);
					}
					$this->obj->DB_delete_all("ad_order","`id` in(".@implode(',',$del).")  and `status`<>0","");
		    	}else{
		    		$this->obj->DB_delete_all("ad_order","`id`='$del' and `status`<>0");
		    	}
