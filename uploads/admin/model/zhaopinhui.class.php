...
class zhaopinhui_controller extends adminCommon{
	...
	function del_action(){
		...
		$zph=$this->obj->DB_select_all("zhaopinhui","`id` in ($linkid)","is_themb,banner");
		if(is_array($zph)){
			foreach($zph as $v){
	...
	function save_action(){
		...
		unset($_POST['add']);
		unset($_POST['id']);
		$info=$this->obj->DB_select_once("zhaopinhui","`id`='".$id."'",'`did`,`is_themb`,`banner`');
		if($_POST['is_themb']!=$info["is_themb"]){
			$_POST['is_themb'] = $this->checksrc($_POST['is_themb'], $info["is_themb"]);
		}
		if($_POST['banner']!=$info["banner"]){
			$_POST['banner'] = $this->checksrc($_POST['banner'], $info["banner"]);
		}
		if($_POST['time']){
	...
	function upload_action(){
		...
		$urlarr['page']="{{page}}";
		$pageurl=Url($m,$urlarr,'admin');
		$rows=$this->get_page("zhaopinhui_pic","zid='".$id."'",$pageurl,$this->config['sy_listnum']);
		if(is_array($rows)){
	...
	function setthemb_action(){
		if($_POST['id']){
			$row=$this->obj->DB_select_once("zhaopinhui_pic","`id`='".$_POST['id']."'","`zid`,`pic`,`id`");
			if($row['pic']&&file_exists(APP_PATH.$row['pic'])){
				$this->obj->DB_update_all("zhaopinhui_pic","`is_themb`=' '","`zid`='".$row['zid']."'");
				$this->obj->DB_update_all("zhaopinhui","`is_themb`='".'.'.$row['pic']."'","`id`='".$row['zid']."'");
				$this->obj->DB_update_all("zhaopinhui_pic","`is_themb`='1'","`id`='".$row['id']."'");
	...
	function com_action(){
		...
		$rows=$this->get_page("zhaopinhui_com",$where,$pageurl,$this->config['sy_listnum']);
		if(is_array($rows)){
	...
	function comxls_action(){
		...
		if($_POST['zid']&&$_POST['cid']){
			...
			$rows = $this->obj->DB_select_all("zhaopinhui_com","`id` in(".$_POST['cid'].")");
			...
			if(!empty($rows)){
				...
				$companys = $this->obj->DB_select_all("company","`uid` in (".@implode(",",$comids).")","`uid`,`name`,`address`,`linkphone`,`linkman`,`linktel`,`content`,`welfare`,`mun`,`money`,`moneytype`");
				
				
				$jobs = $this->obj->DB_select_all("company_job","`id` in (".@implode(",",$jobids).") GROUP BY id","`id`,`uid`,`name`,`number`,`minsalary`,`maxsalary`,`exp`,`edu`,`provinceid`,`cityid`,`three_cityid`, count(uid) as `num`");
				...
				if(is_array($jobs)){
					...
					foreach($jobs as $k1=>$v1){
						...
						if(!empty($v1['maxsalary'])){
							...
							$jobs[$k1]['salary']	= "￥".$v1['minsalary'].'以上';
						}else{
							$jobs[$k1]['salary']	= "面议";
						}
				...
				$this->MODEL('log')->admin_log("导出报名招聘会信息");
	...
	function shjobmsg($pid,$yesid,$statusbody){
	    ...
	    if($yesid==1){
	        $data['type']="zphshtg";
			$content="招聘会《".$zph['title']."》审核通过";
	    }elseif($yesid==2){
	        $data['type']="zphshwtg";
			$content="招聘会《".$zph['title']."》审核未通过";
	    }
		if($content){
			$this->automsg($content,$zphcom['uid']);
		}
	    if($data['type']!=""){
	...
	function getjoblist_action(){
	    ...
	    $rows = $this->obj->DB_select_all("company_job","`uid`='".$comid."' and `r_status`<>'2' and `status`<>'1' and `state`=1","`id`,`name`");
	    $html = '';
	    foreach ($rows as $v){
