...
class admin_question_controller extends adminCommon{
	...
	function index_action()
	{
		...
		$mids=@implode(',',$mid);
		$cids=@implode(',',$cid);
		$member=$this->obj->DB_select_all("member","`uid` in(".$mids.")","`uid`,`username`");

		$q_class=$this->obj->DB_select_all("q_class","`id` in(".$cids.")","`id`,`name`");
	...
	function status_action(){
		 ...
		 if(is_array($id)){
			...
			$list=$this->obj->DB_select_all("question","`id` IN ($aid)","`uid`,`title`");
			if($list){
				foreach($list as $v){
					if($status==1){
						$this->automsg("问答《".$v['title']."》审核通过",$v['uid']);
					}elseif($status==2){
						$this->automsg("问答《".$v['title']."》审核未通过",$v['uid']);
					}
				}
			}
