...
class report_controller extends adminCommon{
	...
	function index_action(){
		...
		if($type=='0' || $type==''){
			...
			$pageurl=Url($_GET['m'],$urlarr,'admin');
			$q_report=$this->get_page("report",$where,$pageurl,$this->config['sy_listnum']);
			if(is_array($q_report)&&$q_report){
				foreach($q_report as $key=>$value){
					$reason=@explode('@',$value['r_reason']);
					$q_report[$key]['ereason']=$reason[0];
					$q_report[$key]['rreason']=$reason[1];
				}
			}
			$this->yunset("q_report",$q_report);
		}
	...
	function showxjh_action(){
		if($_POST['id']){
			$row=$this->obj->DB_select_once("report","id='".$_POST['id']."' and type='3'","r_reason");
