...
class emailconfig_controller extends adminCommon{
	...
	function save_action(){
 		if($_POST['config']){
			$config=$this->obj->DB_select_num("admin_config","`name`='sy_email_online'");
			if($config==false){
				$this->obj->DB_insert_once("admin_config","`name`='sy_email_online',`config`='1'");
			}else{
				$this->obj->DB_update_all("admin_config","`config`='1'","`name`='sy_email_online'");
			}
			$this->web_config();
		...
		if($_POST['aliconfig']){
			...
			if($config==false){
				$this->obj->DB_insert_once("admin_config","`name`='sy_email_set',`config`='1'");
			}else{
				$this->obj->DB_update_all("admin_config","`config`='1'","`name`='sy_email_set'");
			}
			$this->web_config();
			$this->ACT_layer_msg("邮件服务器设置成功！",9,1,2,1);
		}
	...
	function ceshi_action(){
 		if($_POST["ceshi_email"]){
			...
			$emailData['content'] = "恭喜你，该邮件帐户可以正常使用<br> ".$this->config['sy_webname'];
