...
class statis_user_controller extends adminCommon{
	...
	private function getTotal($timeBegin = '', $timeEnd = '', $userType = '')	{
		
		$where = 'order_state = 2';

 		$member = $this->obj->DB_select_all("member","`usertype`='".$userType."'","`uid`");
		foreach($member as $v){
			$uid[]=$v['uid'];
		}
	...
	public function getUser($userType){	
		...
		list($in, $out, $net_income) = $this->getTotal( $timeBegin, $timeEnd, $userType);
