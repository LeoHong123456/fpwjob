...
class invoice_controller extends adminCommon{
	...
	function status_action(){
		...
		$list=$this->obj->DB_select_all("invoice_record","`id` in (".$_POST['pid'].")",'invoicetitle,uid');
		if($list){
			foreach($list as $v){
				if($_POST['status']==1){
					$this->automsg("发票《".$v['invoicetitle']."》审核通过",$v['uid']);
				}elseif($_POST['status']==2){
					$this->automsg("发票《".$v['invoicetitle']."》审核未通过",$v['uid']);
				}elseif($_POST['status']==3){
					$this->automsg("发票《".$v['invoicetitle']."》已打印",$v['uid']);
				}elseif($_POST['status']==4){
					$this->automsg("发票《".$v['invoicetitle']."》已邮寄",$v['uid']);
				}
			}
		}
