...
class admin_lt_job_controller extends adminCommon{
	function set_search(){
		...
		$search_list[]=array("param"=>"status","name"=>'审核状态',"value"=>array("1"=>"已审核","3"=>"未通过","4"=>"未审核"));
	...
	function index_action(){
		...
		if($_GET['status']){
			if($_GET['status']=="1"){
				$where.= " and `edate`>'".time()."'and `status`=1";
		...
		$rows=$this->get_page("lt_job",$where,$pageurl,$this->config['sy_listnum']);
	...
	function shjobmsg($jobid,$yesid){
		...
		if($yesid==1){
			$content="职位《".$comarr['job_name']."》审核通过";
		}elseif($yesid==3){
			$content="职位《".$comarr['job_name']."》审核未通过";
		}
		if($content){
	...
	function recommend_action(){
		...
		$job=$this->obj->DB_select_once("lt_job","`id`='".$_GET['id']."'","job_name,uid");
		if($nid && ($_GET['rec']==1)){
			$this->obj->DB_update_all("lt_job","`rec_time`='".time()."'","`id`='".$_GET['id']."'");
			$this->automsg("管理员操作：推荐职位《".$job['job_name']."》",$job['uid']);
		}elseif($nid && ($_GET['rec']==0)){
			$this->obj->DB_update_all("lt_job","`rec_time`=''","`id`='".$_GET['id']."'");
			$this->automsg("管理员操作：取消职位《".$job['job_name']."》推荐",$job['uid']);
		}
		echo $nid?1:0;die;
	}
	...
	function del_action()
	{
	    ...
	    if($_GET['del'])
	    {
			...
			$list=$this->obj->DB_select_all("lt_job","`id` in (".$del.")","`uid`,`job_name`");
			...
			if($list){
				foreach($list as $v){
					$this->automsg("管理员操作：删除职位《".$v['job_name']."》",$v['uid']);
				}
			}
