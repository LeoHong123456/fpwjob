...
class advertise_controller extends adminCommon{
	...
	function index_action(){
		...
		if(is_array($linkrows)){
			foreach($linkrows as $key=>$value){
				...
				if($value['pic_url']&&file_exists(str_replace('./',APP_PATH,$value['pic_url']))){
 					$value['pic_url']=str_replace('./','../',$value['pic_url']);
				}
	...
	function class_action(){
		...
		$urlarr['page']="{{page}}";
		$pageurl=Url($_GET['m'],$urlarr,'admin');
		$ad_class_list=$this->get_page("ad_class",$where." order by id desc",$pageurl,$this->config['sy_listnum']);
		if($ad_class_list && is_array($ad_class_list)){
			foreach($ad_class_list as $k => $v){
				if($v['href']&&file_exists(str_replace('./',APP_PATH,$v['href']))){
					$ad_class_list[$k]['hrefn']=str_replace('./', $this->config['sy_weburl'].'/', $v['href']);
				}
			}
		}
		if($_GET['ad_id']){
	...
	function addclass_action(){
		if($_POST['class_name']){
			$adclass=$this->obj->DB_select_once('ad_class',"`id`='".$_POST['id']."'","`href`");
			
			if($_POST['href']!=$adclass['href']){
				$href = $this->checksrc($_POST['href'],$adclass['href']);
			}else{
				$href = $adclass['href'];
			}

			if($_POST['type']==1){
				$data="`integral_buy`='".$_POST['integral_buy']."',`class_name`='".$_POST['class_name']."',`orders`='".$_POST['orders']."',`href`='".$href."',`type`='".$_POST['type']."',`btype`='".$_POST['btype']."',`x`='".$_POST['x']."',`y`='".$_POST['y']."',`remark`='".$_POST['remark']."'";
			}else{
				$data="`integral_buy`='',`class_name`='".$_POST['class_name']."',`orders`='".$_POST['orders']."',`href`='',`type`='".$_POST['type']."',`btype`='',`x`='',`y`='',`remark`=''";
				unlink_pic(str_replace('./', APP_PATH.'/',$adclass['href']));
			}
			if($_POST['id']){
		...
		if($_GET['id']){
			...
			if($info['href']&&file_exists(str_replace('./',APP_PATH,$info['href']))){
				$info['hrefn']=str_replace('./',$this->config['sy_weburl'].'/',$info['href']);
			}
			$this->yunset('info',$info);
		}
	...
	function delclass_action(){
	    	...
	    	if($del){
				if(is_array($del)){
					...
					if(is_array($ad)){
						...
						$class = $this->obj->DB_select_all("ad_class","`id` in(".@implode(',',$del).")","`href`");
						if($class){
							foreach($class as $v){
								unlink_pic(str_replace('./', APP_PATH.'/',$v['href']));
							}
						}
		...
		if(is_array($ad)){
			...
			$class= $this->obj->DB_select_once("ad_class","`id`='".$id."'","`href`");
			unlink_pic(str_replace('./', APP_PATH.'/',$class['href']));
	...
	function adbuyshow_action(){
		if($_POST['id']){
			$info=$this->obj->DB_select_once("ad_class","`id`='".intval($_POST['id'])."'");
			if($info['href']&&file_exists(str_replace('./',APP_PATH,$info['href']))){
				$info['hrefn']=str_replace('./',$this->config['sy_weburl'].'/',$info['href']);
			}
	...
	function delbuy_action(){
		$this->check_token();
	    if(isset($_GET['id'])){
			$adclass=$this->obj->DB_select_once('ad_class',"`id`='".$_GET['id']."'","`href`");
			if($_POST['href']!=$adclass['href']){
				$href = $this->checksrc($_POST['href'],$adclass['href']);
			}else{
				$href = $adclass['href'];
			}
			$data="`integral_buy`='',`href`='',`type`='2',`btype`='',`x`='',`y`='',`remark`=''";
			unlink_pic(str_replace('./', APP_PATH.'/',$adclass['href']));
			$result=$this->obj->DB_update_all("ad_class",$data,"`id`='".$_GET['id']."'");
			if($result){
