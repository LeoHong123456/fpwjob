	...
	function del_action(){
	    if($_POST['del']){
	    	...
	    	if($del){
				...
				$list=$this->obj->DB_select_all("company_msg","`id` in (".$del.")","`uid`");
	    ...
	    if(isset($_GET['id'])){
			$this->check_token();
			$info=$this->obj->DB_select_all("company_msg","`id`='".$_GET['id']."'","`uid`");
			$result=$this->obj->DB_delete_all("company_msg","`id`='".$_GET['id']."'" );
			$this->automsg('管理员操作：删除会员（ID：'.$info['uid'].'）面试评论',$info['uid']);
