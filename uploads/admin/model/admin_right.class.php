...
class admin_right_controller extends adminCommon{
	...
	function ajax_statis_action(){
		...
		$comm=$this->obj->DB_select_all("member","`status`=0 and `usertype`='2'","`uid`");
 		foreach($comm as $val){
			$comuids[]=$val['uid'];
		}
		$checkCompanyNum = $this->obj->DB_select_num("company","`uid` in (".@implode(',',$comuids).")");
		...
		$resumeNumMon = $this->obj->DB_select_num('resume_expect', "`ctime` >= {$month}");
		$jobNumMon = $this->obj->DB_select_num('company_job', "`sdate` >= {$month}");
		$companyNumMon = $this->obj->DB_select_num('member', "`usertype`='2' and `reg_date` >= {$month}");
		$userNumMon = $this->obj->DB_select_num('member', "`usertype`='1' and `reg_date` >= {$month}");
		$newsNumMon = $this->obj->DB_select_num('news_base', "`datetime` >= {$month}");
		$ggNumMon = $this->obj->DB_select_num('adclick', "`addtime` >= {$month}");
		$onceNumMon = $this->obj->DB_select_num('once_job', "`ctime` >= {$month}");
		$tinyNumMon = $this->obj->DB_select_num('resume_tiny', "`time` >= {$month}");
	...
	function traintz_action(){
		$this->rz("member_log","ctime","培训会员日志","usertype=4 order by ctime desc");
	}
