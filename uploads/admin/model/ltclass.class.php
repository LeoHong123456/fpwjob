 ...
 */
class ltclass_controller extends adminCommon{
	...
	function save_action(){
		...
		$ltclass=$this->obj->DB_select_all("ltclass","`name` in ('".implode("','", $name)."')".$where);
		if(empty($ltclass)){
			...
			$this->cache_action();
			$add?$msg=2:$msg=3;
			$this->MODEL('log')->admin_log("猎头会员分类(ID:".$add.")添加成功");
		}else{
			$msg=1;
		}
		echo $msg;die;
	...
	function ajax_action()
	{
		if($_POST['sort'])
		{
			...
			$this->MODEL('log')->admin_log("猎头会员分类(ID:".$_POST['id'].")排序修改成功");
		}
		if($_POST['name'])
		{
			$this->obj->DB_update_all("ltclass","`name`='".$_POST['name']."'","`id`='".$_POST['id']."'");
			$this->MODEL('log')->admin_log("猎头会员分类(ID:".$_POST['id'].")名称修改成功");
		}
