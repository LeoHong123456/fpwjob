...
class comnews_controller extends adminCommon{
	...
	function status_action(){
		...
		if(is_array($id)){
			...
			$list=$this->obj->DB_select_all("company_news","`id` IN ($aid)","`uid`,`title`");
			if($list){
				foreach($list as $v){
					if ($status=='1'){
						$this->automsg('企业新闻《'.$v['title'].'》审核通过',$v['uid']);
					}elseif ($status=='2'){
						$this->automsg('企业新闻《'.$v['title'].'》审核未通过',$v['uid']);
					}
				}
			}
	...
	function del_com($id)
	{
		...
		if($id_arr[0])
		{
			$news=$this->obj->DB_select_once("company_news","`id`='".$id_arr[0]."'");
			$result=$this->obj->DB_delete_all("company_news","`id`='".$id_arr[0]."'" );
			$this->automsg('管理员操作：删除企业新闻《'.$news['title'].'》',$news['uid']);
		}
		return $result;
	}
}
