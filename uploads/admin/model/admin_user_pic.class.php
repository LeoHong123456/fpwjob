...
class admin_user_pic_controller extends adminCommon{
	...
	function index_action(){
		...
		if($_GET['status']=='3'){
			...
			$where.=" and `photo_status`='".$_GET['status']."'";
	        $urlarr['status']=$_GET['status'];
		}
		$where.=" order by `photo_status` desc, `uid` desc";
		$urlarr['page']="{{page}}";
	...
	function getStatusBody_action(){
		$resume = $this->obj->DB_select_once("resume","`uid`=".$_GET['uid'],"`photo_statusbody`");
		echo trim($resume['photo_statusbody']);die;
	}

	function status_action(){
		extract($_POST);
		$statusbody = trim($_POST['statusbody']);
 		$id=$this->obj->DB_update_all("resume","`photo_status`='".$_POST['status']."',`photo_statusbody`='".$statusbody."'","`uid` in (".$uid.")");
		$list=$this->obj->DB_select_all("resume","`uid` in (".$uid.")","`uid`,`photo`");
		
		if($list){
			if($id && $_POST['status']==0){
				foreach($list as $v){
					$this->obj->DB_update_all("resume_expect", "`photo`='".$v['photo']."'" , "`uid`='".$v['uid']."'");
					$this->obj->DB_update_all("answer", "`pic`='".$v['photo']."'" , "`uid`='".$v['uid']."'");
					$this->obj->DB_update_all("question", "`pic`='".$v['photo']."'" , "`uid`='".$v['uid']."'");
				}	
			}
			foreach($list as $v){
				if($_POST['status']==0){
 		...
 		$id?$this->ACT_layer_msg("个人头像审核(ID:".$uid.")设置成功！",9,$_SERVER['HTTP_REFERER'],2,1):$this->ACT_layer_msg( "设置失败！",8,$_SERVER['HTTP_REFERER']);
	}
	...
	function del_action(){
		if($_GET['delid']){
			...
			if($_GET['type']=='photo'){
				...
				unlink_pic(str_replace("./",APP_PATH."/",$row['photo']));
				$delid=$this->obj->DB_update_all("resume","`photo`='',`resume_photo`='',`photo_status`=''","`uid`='".$_GET['delid']."'");
				$this->obj->DB_update_all("resume_expect","`photo`='',`phototype`='0'","`uid`='".$_GET['delid']."'");
