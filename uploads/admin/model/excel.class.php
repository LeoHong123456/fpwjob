				...
				if($value[name]!=""){
					...
					if($uid){
						...
						if($uid){
						    ...
						    if ($job_classid){
						        $resumeM->addJobclass($resumeid,$uid,$job_classid);
						    }
						    if ($city_classid){
						        $resumeM->addCityclass($resumeid,$uid,$city_classid);
						    }
							...
							if($resumeid  && $value['work_n']!=""){
								...
								$this->obj->DB_update_all("resume_expect",$resume_time,"`uid`='$uid'");
							}
			...
			foreach($user as $key=>$value){
				...
				if($uid){
					...
					if($value['jobclass']!=""){
						...
						$jobval.=",`com_name`='$comname',`mun` = $mun,`source`='7',`r_status`='1'";
						$numjob++;
