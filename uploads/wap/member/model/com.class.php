...
class com_controller extends wap_controller{
	...
	function hr_action(){
		...
		if(is_array($rows) && !empty($rows)){
			...
			$userrows=$this->obj->DB_select_all("resume","`uid` in (".pylode(",",$uid).") and `r_status`<>'2'","`def_job` as `id`,`name`,`sex`,`edu`,`uid`,`exp`,`nametype`");
					...
					foreach($userrows as $val){
						if($v['uid']==$val['uid']){
							if($this->config['user_name']=='1'){
								if($val['nametype']==3){
									if($val['sex']==1){
										$rows[$k]['name'] = mb_substr($val['name'],0,1,'utf-8')."先生";
									}else{
										$rows[$k]['name'] = mb_substr($val['name'],0,1,'utf-8')."女士";
									}
								}elseif($val['nametype']==2){
									$rows[$k]['name'] = "NO.".$val['id'];
								}else{
	...
	function hrset_action(){
		...
		$browse=(int)$_GET['is_browse'];
 		...
 		if($id&&$browse){
 		    $row = $this->obj->DB_select_once("userid_job","`id`='".$id."' and `com_id`='".$this->uid."'","`uid`,`eid`,`job_id`,`type`");
 		    if($row['type']==1){
			...
			if($browse==4){
				$resumeexp=$this->obj->DB_select_once("resume_expect","`id`='".$row['eid']."' and `r_status`<>'2' and `status`='1'",'uid,uname');
				$uid=$this->obj->DB_select_once("resume","`uid`='".$resumeexp['uid']."'","telphone,email");
				if($row['type']==1){
				    $comjob=$this->obj->DB_select_once("company_job","`uid`='".$this->uid."' and `id`='".$row['job_id']."'","name,com_name");
				}elseif($row['type']==2){
				    $comjob=$this->obj->DB_select_once("lt_job","`uid`='".$this->uid."' and `id`='".$row['job_id']."'","`job_name` as `name`,com_name");
				}
		...
		if($_POST['price']){
			...
			if($_POST['comvip']){
				...
				$data['type']='1';
					...
					$time=time();
          $statis = $this->obj->DB_select_once("company_statis","`uid`='".$this->uid."'");
          if($statis['rating']==3 && $statis['vip_etime']==0 || $statis['vip_etime']>$time){
          ...
          $this->yunset("layer",$data);
			    ...
			    if($integralid&&$discount>0){
					//判断充值积分与对应的折扣是否合理
					if($_POST['price_int'] <= $CacheMclass['integralclass_name'][$integralid])
					{
						$_POST['price_int']	=	$CacheMclass['integralclass_name'][$integralid];
	...
	function look_job_action(){
		...
		if(is_array($rows)){
			...
			$resume=$this->obj->DB_select_all("resume","`uid` in (".pylode(",",$uid).")","`uid`,`name`,`edu`,`exp`,`sex`,`nametype`,`def_job` as `eid`");
	...
	function look_resume_action(){
		...
		if(is_array($rows)){
			...
			$resume=$this->obj->DB_select_alls("resume","resume_expect","a.uid=b.uid and b.`id` in (".pylode(",",$resume_id).")","a.`name`,a.`sex`,a.`exp`,a.`edu`,a.`birthday`,a.`nametype`,b.`id`,b.job_classid,b.`salary`");
			$userid_msg=$this->obj->DB_select_all("userid_msg","`fid`='".$this->uid."' and `uid` in (".pylode(",",$uid).")","uid");
			if(is_array($resume)){
				include(PLUS_PATH."user.cache.php");
				include(PLUS_PATH."job.cache.php");
	...
	function talent_pool_action(){
		...
		if(is_array($rows)) {
			...
			$resume=$this->obj->DB_select_alls("resume","resume_expect","a.uid=b.uid and a.`r_status`<>'2' and a.uid in (".pylode(',',$uid).")","a.`name`,a.`uid`,a.`sex`,a.`birthday`,a.`nametype`,b.`edu`,a.`exp`,b.`job_classid`,b.id as eid,b.salary");
	...
	function invite_action(){
		...
		if(is_array($rows) && !empty($rows)){
			...
			$resume=$this->obj->DB_select_all("resume","`uid` in (".pylode(",",$uid).") and `r_status`<>'2'","`uid`,`name`,`exp`,`sex`,`edu`,`nametype`,`def_job` as `eid`");
			foreach($resume as $val){
				$eid[]=$val['eid'];
			}
			...
			if(is_array($resume)){
				...
				foreach($resume as $val){
					...
					if($this->config['user_name']=='1' || !$this->config['user_name']){
						if($val['nametype']==3){
							if($val['sex']==1){
								$user[$val['uid']]['name'] = mb_substr($val['name'],0,1,'utf-8')."先生";
							}else{
								$user[$val['uid']]['name'] = mb_substr($val['name'],0,1,'utf-8')."女士";
							}
						}elseif($val['nametype']==2){
							$user[$val['uid']]['name'] = "NO.".$val['eid'];
						}else{
	...
	function attention_me_action(){
		...
		$defineJobs = $this->obj->DB_select_all("resume",$whereResume," `uid`,`name`,`def_job`,`birthday`,`sex`,`nametype`");
